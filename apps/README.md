...
**🔉 ウィスパード英語ボタン（全機能共通）**：どのタブ／モードにも **「🔉 ウィスパード英語」ボタン**が
あり、**発声せず**入力した**ウィスパード英語**（母音を落とした・部分的な英語）を、子音スケルトン照合で
**完全な英語に復元**してその欄に入力します（silent-talk 超えの精度）。全機能でウィスパード英語モードが
使えます。共有 Java コア `Whisper.verbalizeEn(cue)`。

**🔉 Bada Vim のウィスパード英語挿入モード（画面全体・直接・短文でない）**：**⑧ Bada Vim** タブでは、
ノーマルモードの `W` または **「🔉 ウィスパード英語挿入」ボタン**で **`-- WHISPER INSERT --`** に入り、
**画面全体に**発声せず**ウィスパード英語**を直接打鍵します。`Esc` を押すとその行が **`silent-talk 超え
精度`で完全な英語へ復元**され、`precision X% > silent-talk 92.0%` を表示します（別に短文入力ではありません）。
