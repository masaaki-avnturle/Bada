...
**🧠 思考入力ボタン（全機能共通）**：どのタブ／モードにも **「🧠 思考入力」ボタン**があり、
発声せず入力した手がかりを、大域的部分積分多様体をゲージにした Mind トランスフォーマーが
**silent-talk 超えの精度**でその欄に言語化します（宇宙電信＝送信文、擬似QC＝QASM プログラム、
思考言語化＝信号、コード生成＝意図）。共有 Java コア `SilentTalk.thoughtFill(cue, kind)`。
