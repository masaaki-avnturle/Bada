...
操作: **PDFを読み込む** か本文を貼り付け → モード選択 → 開始CPM / 加速 /
単位文字数 を設定 → **開始 / 停止**。

### PDF読み込み

- 「**PDFを読み込む**」ボタンから、自分で作った PDF を選択すると、本文の
  テキストを抽出して読み込みます（`pdfbox-android` を使用）。
- PDF の **ページ区切りはそのまま SRS のページ送り**になります（内部で
  form-feed に変換）。抽出した複数行のブロック文章を、**SRS速読・瞬読の
  両モード**でそのまま使えます。
- 文字が埋め込まれたテキスト PDF に対応します。**画像としてスキャンされた
  PDF**（文字情報なし）は抽出できません（別途 OCR が必要）。
