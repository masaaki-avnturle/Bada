...
## 📺 インターネットTV：本当の動画を流し込む

各ホログラム画面で、**インターネット上の“本当の動画”**（無料 / クリエイティブ・コモンズ配信）を
再生します。海賊版や他者放送の無断配信は使わず、自由に利用できる公開動画のみを
「インターネットTVチャンネル」として使用します。

- 収録チャンネル例（Blender オープンムービー CC-BY / Google 公開サンプル）:
  NATURE=Big Buck Bunny・FANTASY=Sintel・SCI-FI=Tears of Steel・DREAM=Elephants Dream ほか。
- 実映像はホログラフィック演出（色調ティント・走査線・グロー・奥行き・視差・立体視）を保ったまま再生。
- 「📺 実映像 / エンジン映像」で切替、「🔊 音声」で焦点チャンネルの音を再生（自動再生対策でユーザー操作時に解除）。
- **オフライン / 読み込み不可時は自動でエンジン生成映像にフォールバック**（ネットが無くても動作）。
- デコード負荷対策：焦点付近のチャンネルのみ再生し、遠方は一時停止。

## ✨ 追加機能（3点）

- **① Ch.T 速報検索の流し込み** — 上部の検索バーに語（地震 / 経済 / 宇宙 …）を入れると、
  その速報が生成され、ホログラムのパネル群に直接反映されます。
- **② ジャイロ視差** — 「🎛 モーション」を有効化すると、端末の傾きで視点（視差）が動きます
  （iOS 13+ は許可ダイアログ対応。PC ではポインタで視差）。
- **③ 立体視 / VR（SBS）** — 「◨◧ 立体視」で左右2眼のサイドバイサイド表示。端末を
  VR ゴーグル（カードボード等）に入れると立体的に見えます。WebXR 対応端末では「VR」表記。
  「× 立体視を終了」で戻ります。
