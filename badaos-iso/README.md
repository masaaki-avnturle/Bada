...
- **起動方式**: BIOS(isolinux)+ UEFI(GRUB)ハイブリッド ISO → **Rufus でそのまま USB に書ける**
...
| ファイル | 内容 |
|:---|:---|
| `BadaVMPro-OS-1.0.0-amd64.iso` | 起動可能 ISO 本体 |
| `BadaVMPro-OS-1.0.0-amd64.iso.sha256` | 検証用チェックサム |
...
2. **「選択」**で `BadaVMPro-OS-1.0.0-amd64.iso` を指定
...
> macOS / Linux では `sudo dd if=BadaVMPro-OS-1.0.0-amd64.iso of=/dev/sdX bs=4M status=progress conv=fsync` でも可(ISO は isohybrid)。

## 🖥 起動後

1. GRUB/isolinux メニューで **Bada VM Pro OS** を選択 → w9wm デスクトップに自動ログイン(ユーザー `bada`)
