...
class TestIQTabletDerived < Minitest::Test
  ...
  def test_skin_excess_neutral_zero
    # neutral skin (ir=33, ambient=23, Δ=10) -> external skin excess 0
    assert_in_delta 0.0, Bada::IQ.vm.call("skin_excess", [33.0, 23.0]), 1e-9
  end

  def test_skin_excess_rises_with_surface
    assert_operator Bada::IQ.vm.call("skin_excess", [38.0, 22.0]), :>, 0.0
    assert_operator Bada::IQ.vm.call("skin_excess", [30.0, 23.0]), :<, 0.0
  end

  def test_neutral_skin_gives_population_mean
    # skin excess 0 -> all derived z = 0 -> posterior at the prior mean
    assert_in_delta 100.0, Bada::IQ.assess_thermal_derived(33.0, 23.0).iq, 1e-6
  end

  def test_report_shows_skin_temperature
    assert_includes Bada::IQ.assess_thermal_derived(36.6, 23.0).report, "皮膚温度"
  end
end
