...
でそれぞれの生成物をカーソル位置に挿入します。デスクトップの**⑦ Bada Vim タブ**は**画面全体が
長長文バッファ**の本格モーダルエディタで、`-- NORMAL --`／`-- INSERT --` モード表示つき（`i` で
インサート＝直接長長文を打鍵、`Esc` でノーマル、`:` で ex コマンド行）。Android は**⑧ Bada Vim
モード**（`|` 区切りのコマンド列）で同じエンジンを提供します。
