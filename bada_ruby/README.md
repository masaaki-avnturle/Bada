...
**ウィスパード挿入モード**（`-- WHISPER INSERT --`）に入り、行を打鍵して `Esc` を押すとその行が
**復元された完全な英語**に置き換わり `precision X% > silent-talk 92.0%` を表示します。`:whisperen <cue>`
でも同じ復元をカーソル位置に挿入できます。**全機能のソースコード生成**も Vim 内で行えます：
