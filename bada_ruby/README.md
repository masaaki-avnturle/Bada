...
**指示指向への全面移植**: `std/flow.bada`（`seed/map/reduce/mapreduce`）を基盤に、
`entropy`（シャノン＝項に分岐→和で合流）・`manifold`（∬＝ランク分岐→重み付き和で合流）・
`prover`（剰余系を分岐→最初の反例を合流で探索）・`basal`（チャネルを`[添字,視床]`に分岐→
最大で合流＝argmax）を、すべて **`<-` 代入 / `-<` 分岐 / `>-` 合流** の map-reduce で書き
直しました（数値結果は不変）。
