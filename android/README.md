...
**体内温度（核心温）**: 赤外線が読むのは体表（皮膚）温です。そこから **体内温度 T_core**
を推定し（`core_body_temp`: 皮膚は核心より低く、室温が下がるほど皮膚↔核心の差が拡大。
neutral な体表→核心 37.0℃ に較正）、大脳基底核推定モードは**体表ではなく体内温度**
（`T_core − 37℃` の核心温超過）を駆動量として血液/DNA/fMRI/MRI/脳トポを推定します。
