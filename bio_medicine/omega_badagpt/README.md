...
| ④′ | **確実な保存 (全プラットフォーム)** | **APK**: `cordova-plugin-saf-mediastore` で端末の共有 **「ダウンロード」フォルダへ直接保存**(最優先。MediaStore 経由、Android 10+、権限プロンプト不要 — ファイルアプリの Download に即表示)。使えない場合は `cordova-plugin-save-dialog` の標準「保存」ダイアログ → `cordova-plugin-file` の順にフォールバック。**Windows EXE / Ubuntu**: Electron の保存ダイアログを必ず表示(既定は「ダウンロード」フォルダ + 提案ファイル名)し、保存後にフォルダ内でファイルを表示。**ブラウザ**: 従来どおり + data: URI 最終フォールバック。 |
