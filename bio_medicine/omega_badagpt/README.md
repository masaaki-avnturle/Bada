...
| ④′ | **確実な保存 (全プラットフォーム)** | **APK**: Android WebView は通常のダウンロードを無視するため、`cordova-plugin-file` でアプリの外部ストレージ領域へ直接保存し、保存先パスを画面に表示(プラグイン欠落時もフォールバックあり)。**Windows EXE / Ubuntu**: Electron の保存ダイアログを必ず表示(既定は「ダウンロード」フォルダ + 提案ファイル名)し、保存後にフォルダ内でファイルを表示。**ブラウザ**: 従来どおり + data: URI 最終フォールバック。 |
