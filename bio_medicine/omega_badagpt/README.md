...
| ②′ | **質問応答エンジン (`respond.js`)** | アップロード資料を実際に読み、質問・要望に応じて解答を構成:文分割 → キーワード抽出 → ζ-Zipf 重み付き検索で関連文を抽出し、**要約 / 論文(序論・理論・数式・結論)/ 実行可能な単一ファイル HTML アプリ / Python コード / 数式抽出 / 英語要約 / 続きの予知 / レビュー**を資料の根拠付きで生成。Claude API モードでも抽出根拠が自動でプロンプトに添付される。 |
| ② | **2つの生成モード** | ・**ζ-Entropy ローカル**(完全オフライン)<br>・**Claude API + ζ-Entropy**(自分の API キーを入力) |
| ③ | **ζテレメトリ表示** | s, ζ(s), H(s)[nat/bit], 温度 τ, γ, seed をリアルタイム表示。 |
| ①″ | **強化 PDF 抽出器 + Python 変換ツール** | ブラウザ内抽出器がフィルタ連鎖 (Flate/ASCII85/ASCIIHex/RunLength)・日本語 CID フォント (ToUnicode CMap)・UTF-16BE を復号(実レポート PDF 10 本中 9 本を完全テキスト化)。それでも読めない PDF (ToUnicode の無いサブセットフォント等) は案内を表示し、同梱の **`pdf2text.py`**(標準ライブラリのみの Python 変換ツール、アプリ内「Python変換ツールを保存」から取得、`tools/pdf2text.py` にも同梱)で `python3 pdf2text.py 入力.pdf -o 出力.txt` と変換して再アップロードできる。 |
| ①′ | **変換テキストの確認/保存** | アップロードした各ファイル(PDF 含む)は自動でテキスト形式へ変換され、👁 でその場でプレビュー、⬇txt で個別に `.txt` 保存(`report.pdf` → `report.txt`)、「一括 .txt」で全ファイル連結保存ができる。 |
| ④′ | **確実な保存 (全プラットフォーム)** | **APK**: Android WebView は通常のダウンロードを無視するため、`cordova-plugin-save-dialog` で **Android 標準の「保存」ダイアログ**を開き、ユーザーが選んだ場所(ダウンロード フォルダ等)に保存 — 保存したファイルはファイルアプリから必ず見える。ダイアログ未対応・キャンセル時は `cordova-plugin-file` でアプリ外部領域へ直接保存(保存先パスを画面表示)。**Windows EXE / Ubuntu**: Electron の保存ダイアログを必ず表示(既定は「ダウンロード」フォルダ + 提案ファイル名)し、保存後にフォルダ内でファイルを表示。**ブラウザ**: 従来どおり + data: URI 最終フォールバック。 |
| ④ | **成果物をダウンロード** | 生成解答を **PDF** / .txt / .md / **単一ファイル HTML アプリ**(```html ブロックは実行可能アプリとして埋込)/ **抽出ソースコード**として端末(タブレット)に保存。ソースコードは 1 ブロックならソースファイル、**複数ブロックは 1 つの ZIP**(依存なしの内蔵 ZIP 生成器・無圧縮)にまとめて確実に 1 クリックで保存(ブラウザ/WebView は 2 個目以降の自動ダウンロードをブロックするため)。加えてブロックごとの個別保存ボタンも表示。ファイル名は要望の種類(paper / app / code / summary …)+ 要望ごとの一意タグで自動命名。 |
