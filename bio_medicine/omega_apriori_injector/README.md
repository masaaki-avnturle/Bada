...
| `plugins/self_evolve.js` | **自己進化＋一意の機能(UID)**プラグイン: 実行時テロメア自己進化 (`step`)・一意ID (`uid`)・**自己進化できるソースコード**生成 (`source`) |
| `plugins/fpga_functions.js` | **FPGA関数ライブラリ**プラグイン: 自己進化FPGAから生成した任意の名前付き関数を実行/表出。`spec.json` の `fn_library` で運ぶ (`eng.call("fpga_functions","myFn",x)`) |
