...
本アプリは **音響トーン発生器 + センサー可視化 + 娯楽オラクル** です。プリセット名
（リスパダール、ジプレキサ等）は単なる **ラベル** であり、薬剤やその薬効・治療効果を
再現・代替するものではありません。**特定の周波数に医学的・治療的効果があるという
主張はしません。** **脳波計(EEG)・心電図(ECG) 機能も医療機器ではなく、診断には
使えません**（教育・可視化・バイオフィードバック用途）。

**「オラクル(お告げ)」機能は娯楽です。** 未来予知でも「アカシックレコード」への
アクセスでもありません。擬似乱数で選ばれた定型メッセージを表示し、演出として音を
連動させるだけのものです。予知・透視・診断の根拠には一切なりません。治療・服薬の
判断は必ず医師・薬剤師に相談してください。大音量・長時間使用は避けてください。

## Windows 版のダウンロードと実行

Windows 10 / 11 用の単体 `.exe`（インストール不要・追加ランタイム不要）を配布します。

- **GitHub Releases** から: リポジトリの「Releases」→ 最新版の
  `BadaBiofeedback-x64.exe`（64bit）または `BadaBiofeedback-x86.exe`（32bit）。
- **または Actions から**: 「Actions」→ 最新の *Build Bada Biofeedback (Windows exe)*
  → Artifacts の `BadaBiofeedback-windows` をダウンロード。

ダウンロードした `.exe` をダブルクリックで起動します（`waveOut` で音が鳴ります）。
機能: プリセット選択・センサー/EEG/ECG のライブ表示・波形・音再生・オラクル・
脳波/心電フィードバック。

> ⚠️ SmartScreen 警告について: 署名なしのため初回起動時に
> 「WindowsによってPCが保護されました」と表示される場合があります。内容を確認の上、
> 「詳細情報」→「実行」で起動できます（配布元を信頼できる場合のみ）。

### 自分でビルドする場合（Linux 上でクロスコンパイル）
```sh
sudo apt-get install -y gcc-mingw-w64-x86-64 gcc-mingw-w64-i686 make
cd bio_medicine/biofeedback_audio
...
| `oracle.{c,h}` | 娯楽オラクル（乱数で定型メッセージを選ぶ決定的関数。**予知ではない**） |
| `rt_audio.{c,h}` | リアルタイム音声出力（ALSA直結、別スレッドで連続再生） |
| `biofeedback_audio.c` | **CLI** フロントエンド（`bio` で EEG/ECG モニタ、`oracle` でお告げ） |
| `gui_ncurses.c` → `bfa_tui` | **端末GUI**（ncurses） |
| `gui_x11.c` → `bfa_gui` | **X11 グラフィカルGUI** |
| `gui_gtk.c` → `bfa_gtk` | **GTK+3 グラフィカルGUI** |
| `gui_win32.c` → `BadaBiofeedback-*.exe` | **Windows ネイティブGUI**（Win32 + WinMM、mingw でクロスビルド） |
