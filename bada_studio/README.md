...
- **mp3 / mp4 → json に変換 (v1.3)** — 「🎼 json に変換」で音を採譜: 打鍵 (スペクトル・フラックス) で区切り、各区間の強い倍音の山を音にし (倍音を抑制、最大 4 声)、4 秒ごとの和音をクロマから求めて score.json (拡張子 .json) に保存。同時に「📜 楽譜」に読み込まれて編曲でき、元の音は実音の抜粋 (REC) として一緒に鳴る (`transcribe`, `scoreFromTranscription`)
