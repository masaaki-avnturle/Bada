以下は、対象の CVS `.tex,v` ファイルに「上書き」ではなく「追加書き込み（append）」するスクリプトです。既存の内容を残しつつ、CSV の `commitment`（デフォルト）列のテキストを追記します。追記前にバックアップを作るオプションや dry-run、verbose を用意しています。

保存例: `append_commitment_to_texv.py`
...
DEFAULT_CVS_ROOT = "/srv/cvsroot/tuplenetwork/tex/tuplenetst"
...
使い方例
- 検証（書き込みせず）:
  python3 append_commitment_to_texv.py input.csv --dry-run --path-col path --msg-col commit --verbose
- 実行（バックアップを取りながら）:
  python3 append_commitment_to_texv.py input.csv --backup --path-col path --msg-col commit

注意
- RCS の `.tex,v` を単純に追記するとRCSフォーマットを壊す可能性があります。可能であればワーキングコピー `.tex` に追記してから CVS コマンドで正式にコミットする運用を推奨します。直接追記する場合は必ずバックアップを取ってください。
