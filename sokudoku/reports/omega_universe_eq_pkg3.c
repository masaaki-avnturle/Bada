右辺のエラーは、生成される `usr/lang/omegascript.c` 内で `ensure_dir(...)` を呼んでいるのに、その関数を定義していないため発生しています。対処として、`usr/lang/omegascript.c` に独立した `ensure_dir` 実装を追加した修正版の `pkginstallgen.c` を下に示します。これを保存してビルドすれば、Makefile によるコンパイル時の未定義参照は発生しません。

（変更点）
- 生成する `usr/lang/omegascript.c` の先頭に `static int ensure_dir(const char *path)` を追加。
- それに伴う呼び出しはそのままで問題ありません。

修正版（そのままファイルとして保存してコンパイルしてください）：

```c
/* pkginstallgen.c
 *
 * Generates omega_universe_eq_pkg including an analyzer (usr/lang/omegascript.c)
 * that now contains its own ensure_dir implementation to avoid linker errors.
 *
 * Build:
...
static int ensure_dir_local(const char *path) {
  ...
  if (errno == ENOENT) {
    ...
    if (p && p != tmp) {
      *p = '\0';
      if (ensure_dir_local(tmp) == 0) return MKDIR(path) == 0 ? 0 : -1;
    }
  }
  return -1;
}
...
/* usr/lang/omegascript.c with its own ensure_dir implementation */
static const char *OMEGASCRIPT_C_LINES[] = {
  "/* usr/lang/omegascript.c - analyzer with local ensure_dir implementation */",
  "#define _POSIX_C_SOURCE 200809L",
  "#include <stdio.h>",
  "#include <stdlib.h>",
  "#include <string.h>",
  ...
  "#include <errno.h>",
  "",
  "/* local ensure_dir to avoid undefined references when compiled standalone */",
...
/* usr/lang/verifier.py (kept minimal) */
...
int main(int argc, char **argv) {
  ...
  if (ensure_dir_local(target) != 0) { fprintf(stderr, "cannot create %s\n", target); return 1; }
  { char p[4096]; snprintf(p, sizeof(p), "%s/bin", target); ensure_dir_local(p); }
  { char p[4096]; snprintf(p, sizeof(p), "%s/usr/lang", target); ensure_dir_local(p); }
  { char p[4096]; snprintf(p, sizeof(p), "%s/reports", target); ensure_dir_local(p); }
  { char p[4096]; snprintf(p, sizeof(p), "%s/output", target); ensure_dir_local(p); }
  { char p[4096]; snprintf(p, sizeof(p), "%s/output/explanations", target); ensure_dir_local(p); }
...
NULL
  ...
  write_lines(path, readme, 3, 0644);
...
2. ./pkginstallgen
3. cd omega_universe_eq_pkg
4. make

以上で、Make によるビルド時の "undefined reference to 'ensure_dir'" は解消されます。もし別の未定義シンボルが出る場合は、そのエラーメッセージ全文をお願いします。
