以下に、指定された要件を満たすC言語のソースコードを示します。このプログラムは、複数のTeXファイルを結合し、最初のファイルのヘッダー部分をそのまま保持し、2回目以降のファイルのヘッダー部分を`¥section{}`に置き換えます。また、各ファイルの`¥end{document}`は削除し、最後に一つの`¥end{document}`を追加します。
...
### 使用方法
1. 上記のコードをテキストエディタにコピーして、`combine_tex.c`として保存します。
2. Cコンパイラを使用してコンパイルします：", input_files[i]);
                    title[strcspn(title, ".")] = 0; // 拡張子を削除
                    fputs(title, out_fp);
                    fputs("}\n", out_fp);
                }
...
### 使用方法
1. 上記のコードをテキストエディタにコピーして、`combine_tex.c`として保存します。
2. Cコンパイラを使用してコンパイルします：
   ```bash
   gcc combine_tex.c -o combine_tex
   ```
3. プログラムを実行して、出力ファイル名と結合したいTeXファイルを指定します：
   ```bash
   ./combine_tex output.tex input1.tex input2.tex
   ```

このプログラムは、最初のファイルのヘッダー部分をそのまま保持し、2回目以降のファイルの内容を`¥section{}`で囲む形で結合し、最後に一つの`¥end{document}`を追加します。
