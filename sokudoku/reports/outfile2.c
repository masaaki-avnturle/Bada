以下は、要求どおり「ビルド（make）してインストール（make install）できる」C 言語プロジェクトです。ビルドすると実行ファイル `pkggen` が生成され、`make install` を実行すると指定したプレフィックス配下に Omega スクリプト言語のパッケージ（bin, sbin, lib, include, etc, usr）としてファイル群を展開します。

使い方（要約）
- 保存ファイル: `pkggen.c`, `Makefile`
  - ビルド: `make`
  - インストール（デフォルト prefix=/usr/local/virtpkg）: `make install`
  - 別 prefix にインストールする場合: `make install PREFIX=/opt/myprefix`

注意
- インストール時に root 権限が必要な操作（/usr 以下など）をする場合は `sudo make install` を使ってください。
  - 実際の運用には各スクリプトの内容を確認の上、依存（python3, libvirt, OVMF 等）を満たしてください。
...
TARGET = pkggen
  ...
  $(TARGET): pkggen.c
  $(CC) $(CFLAGS) -o $(TARGET) pkggen.c

  install: $(TARGET)
  @echo "Installing package files to $(PREFIX)"
  @mkdir -p $(PREFIX)
  @./$(TARGET) $(PREFIX)
  @echo "Installed to $(PREFIX)"
  @echo "Note: you may need to run 'chmod +x $(PREFIX)/bin/*' and adjust ownership as needed."
...
説明（短く）
  - `pkggen.c` はプレフィックスを受け取り、プレフィックス下にディレクトリを作成してスクリプト類を書き出します（bin, sbin, lib, include, etc, usr）。
- `Makefile` はコンパイルと `make install` を提供。`make install PREFIX=/opt/myprefix` で別の場所にインストールできます。
- 生成されるファイルは Python 実装（前回提供分）をそのまま書き出す形の雛形です。必要なら `pkggen.c` の中の文字列を編集して内容を変更してください。

必要であれば
- 生成する各ファイルの中身をカスタマイズして差し替えます（具体的な修正点を教えてください）。
- インストール時に systemd ユニットを追加、または本当に C 言語で libvirt を直接操作するデーモン実装へ発展させることもできます（要追加仕様）。
