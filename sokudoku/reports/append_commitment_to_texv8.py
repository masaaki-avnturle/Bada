以下は、GitHub で push した `commitment` を含む `comment.csv` の各行を読み、対応する CVS `.tex,v` ファイルに「上書きせず追記（append）」する Python スクリプトです。既存ファイルは上書きせず、末尾へ追記ブロックを追加します。必要に応じてバックアップ作成や dry-run をサポートします。

使い方例:
  python3 append_commitments_to_texv.py --repo /path/to/repo --csv comment.csv --backup-dir backups --verbose
  python3 append_commitments_to_texv.py --csv comment.csv --dry-run

スクリプト: append_commitments_to_texv.py
...
注意事項
- `.tex,v` は RCS 形式ファイルです。直接末尾に追記すると RCS フォーマットの整合性が崩れる可能性があります。本当に履歴を保ちたい場合は RCS コマンド（ci/co）で新しいリビジョンを作る方法を検討してください。本スクリプトは単に「安全に上書きしないで末尾に追記」する用途向けです。
- 追記ブロックのフォーマット（ここでは `%` 行コメント）を必要に応じて変更してください（ファイルの言語やフォーマットに合わせる）。
- 実行前にバックアップを推奨します（--backup-dir オプション）。

必要なら
- RCS コマンドを使って新しいリビジョンとして安全に commit を行う自動化スクリプトを提供します。
