...
\usepackage{chemfig}
\begin{document}
\title{M dimension from catastrophe theory built with \\
M manifold stream from space ideality theory}
...
\maketitle
   ...
   \newcommand{\iiivariint}{%
	\begin{picture}(15,30)
		\put(0,0){
			\scalebox{1}[2]{$\iiint$}}
		\put(0,5){\line(15,0){15}}
                \put(0,4){\line(15,0){15}}
		\put(0,3){\line(15,0){15}}
	\end{picture} %
}
   \newcommand{\iiivar}{%
	\begin{picture}(15,30)
...
\newcommand{\whitewithbodercircleiint}{%
 ...
 \newcommand{\average}{%
  ...
  \newcommand{\intwhitewithnabla}{%
	  \begin{picture}(15,30)
		  \put(0,0){\scalebox{1}[2]{$\int$}}
		  \put(0,0){\scalebox{2.0}[2]{$\nabla$}}
		  \put(3,3){$\nabla$}
	  \end{picture}%
  } 
      ...
      Quantum equation built with being from sqrt and add of possibility
  formula, and this system quantum wave force are squru of bacuum
  put on from space of time theory exceed with fundamental group 
  of qunatum effective reveal system.
  $$ \scalebox{1}[2]{\variiint} 
  [{{\nabla_{i}\nabla_{j} \scalebox{2}[2]{\timebow}} \over {\nabla L}}]^{
	  \ll D(\chi)} [I_m] $$
  $$ \log(x\log x) \ge 2{\sqrt{y\log y}} $$
  $$ x^x = \int [x\log x]e^{-f}dV = \int {\Gamma^{'}(\gamma)}dx_m $$
  $$ = {{[\log(x\log x)]\over {\log x}}} = \int e^{-f}dV $$
  $$ {d \over {d f}}F(x,y) = F^{f^{'}} $$


     This mechanism explain with norm formula is global deprivate equation
  involve with knot theory concerned from Jones manifold.
  $$ ||ds^2|| = \bigotimes||\vee [F^{'}(f(x,y))]|| $$



     M dimension of manifold is what of constructed from universe and other
  dimension, and this system call them recall from space ideality theory.
  $$\iiivar \to  \intwhitewithnabla \to \iiivar $$
  $$ = {{[M_m \scalebox{1}[2]{\text{m}}_M]} \over {G_M}} \wedge{[M_m
  \scalebox{1}[2]{\text{m}}_M]} $$
  $$ = \oint \nabla[{d \over {d M}}]M_m $$
\end{document}
