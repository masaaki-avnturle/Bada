...
\usepackage{amssymb}
\begin{document}
\title{Magic operate with dalia function system}
...
\includegraphics[width=20cm,clip]{realequation5}

    These equations are T tensor scrue with beta function input,
    Equation of timebow is zeta function escourt with beta function in
    logment attachment of being involved.
    $$ T^{\ll(p,q)}, \timebow = {\beta(p,q) \over {\log x}} $$
    This result of function with quantum wave is D-brane and infinty of
    gauss facility.
    $$ = D^{2}\psi \otimes h\nu, [\chi] $$
    This resolved of wave function is norm of space with save integral
    equation.
    $$ ||ds^2|| = \scalebox{1}[2]{\squareiint}_{DM} 
    [D^{2}\psi \otimes h \nu]d\tau $$

    $$ H\Psi = [\chi] $$

    Magic of operator is global integral of timebow equation,
    and this equation esimate with global differential manifold
    to resolve of safty average equation.
    $$\starnabla = \int \timebow^{'}(\square)dx_m  $$
