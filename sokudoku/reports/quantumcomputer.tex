...
\maketitle
      ...
      In mechanism of quantum computer destruct with gas of pressure in
   Jones manifold of calman air straight of condition calcurate with
   a pattern of non possibility deceiede with all of abled in pattern
   is oneselve of possibility of quantum level of particle from
   gas of pressure rout to possibility in D wave mechanism.

   量子コンピュータは、圧力計によるJones多項式のカルマン渦による
   ある不確かな可能性を全体の可能性から差し引いたパターンによる
   全可能性を経路積分から求めて、素粒子レベルを圧力計の経路の
   全パターンから、答えを導く計算をしている。

   実際は、原子の周期を止めて、共有結合と同じ、重ね合わせの原理で、
   不確定性原理の応用で見ている。
   固体量子コンピュータが、この原理であり、光量子コンピュータが、
   上の原理である。液体量子コンピュータは、固体と気体の両方の原理を
   使っている。
   銀河の円盤を、人体の肺の周りの胸郭の原理と同じ、暗号解読の仕組みと
   同型でもある。
...
$$ x^y = {1 \over y^x} $$
 ...
 Therefore zeta function is also constructed with quantum equation.
