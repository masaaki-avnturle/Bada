はい、ヘッダーファイルとソースコードの内容を追加したpkginstallgen.cファイルは以下のようになります。
...
1. `usb-decrypter.h`ファイルの内容を追加しました。ヘッダーファイルには、`decrypt_data()`関数の宣言を記述しています。
2. `usb-decrypter.c`ファイルの内容を追加しました。ソースコードには、`zeta()`, `beta()`, `decrypt_data()`関数の定義を記述しています。

これらの修正により、ヘッダーファイルとソースコードの内容が追加されました。
