...
\usepackage{amssymb}
\begin{document}
\title{Library of akashic recode}
...
\newcommand{\whitewithbodercircleiint}{%
 ...
 \newcommand{\average}{%
  ...
  Zeta function also have with cauchy law equation.
	...
	Cohomogy element owe have with box of D-brane,
	and this equation also have with zeta function on quantum matrix.
	$$ \ll i\hbar \psi \ast H\Psi \gg, 
	 \int \Delta(\zeta)d\zeta $$

	 dia integral manifold also belong with quantum geometry.
	$$ \whitewithbodercircleint (I_m)^{\nabla L} $$
	$$ -2\int{{\nabla_{i}\nabla_{j} (R + \nabla\nabla f)} \over
	{\Delta(R + \Delta)}}dm $$
	Up of equation also have with Rich flow equation on Thurston
	Perelman manifold dimension.
	$$ \Delta(F(\Delta) \times \Delta(G(\Delta))) = - (F(\Delta) \cup
	F(\Delta)) + (F(\Delta) \cap F(\Delta)) $$
	Alfe equation destruct with Thurston Perelman manifold.
	$$ \sum \square(\nabla)[I_m], \secproduct,[\nabla / \square],
	\pi ||\int \nabla_{i}\nabla_{j} f \nabla d \eta||^2 = S^{m}
	\times S^{m-1} $$
	After all, these equation escourt into Jones manifold.
	$$ (\pi r^2) dr_m, {{\partial^{df} (e^{f} + e^{-f})} \over {
		d(e^{f} - e^{-f})}} $$
	$$ \zeta = (at -t^n + a) = e^{f} $$
	Zeta function also have with knot theory selves.
