以下は先のスクリプトを拡張し、PDFファイル（`.pdf`）も検索対象にして中身を表示できるようにした実装です。PDFテキスト抽出には `PyPDF2` を利用します（事前に `pip install PyPDF2` を推奨）。`PyPDF2` が無ければPDFはスキップします。

保存例: `file_search_server_with_pdf.py`
...
ポイント
- PDF抽出は `PyPDF2` を使用。未インストール時はPDFを無視（警告表示）。
- 大きなPDFやスキャン画像のPDFはテキスト抽出できない／不完全な場合があります。OCR が必要なら別途 `pytesseract` 等の導入が必要です。
