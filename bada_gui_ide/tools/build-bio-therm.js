...
<header>
  <div><div class="logo">🧬 BioThermNet — 人間に近い生命体ファインダー</div>
    <div class="sub">Γ関数の大域的積分多様体の機知 × Jones多項式の熱感知 — 人間と同型の生命体が成立し、子孫(世代)を残し続けられる惑星を実測物理で発見する</div></div>
</header>
<main>
...
(function(){
  ...
  DATA.forEach(function(p){ p.lin=lineage(p); p.his=humanScore(p);
  ...
  function renderList(){
    ...
    DATA.forEach(function(p,i){
      ...
      d.innerHTML="<span class='swatch' style='background:"+heatColor(p.teq)+"'></span><b>"+(p.earth?"":("#"+i+" "))+p.name+"</b>"+(!p.earth&&p.lin.canEvolve?" 👪":"")+
