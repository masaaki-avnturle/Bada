...
<header>
  ...
  <span id="deskBadge" style="display:none;background:#173a29;border:1px solid #2e6b46;border-radius:8px;padding:5px 10px;font-size:12px;color:#8ff0c0"></span>
...
(function(){
  ...
  function commit(rerender){ save(binds); regen(); syncDesktop(); if(rerender) renderRows(); }

  /* ---------- desktop (Electron) bridge: register real global hotkeys ----------
     window.madokey is injected by the MadoKey desktop app's preload. In a plain
     browser it is undefined, so this is a no-op. */
  function syncDesktop(){
    var badge=$("deskBadge");
    if(window.madokey && typeof window.madokey.setBinds==="function"){
      var active=binds.filter(function(b){ return (b[0]||"").trim(); });
      try{ window.madokey.setBinds(active); }catch(e){}
      if(badge){ badge.style.display="inline-block";
        badge.textContent="🖥 デスクトップ版: "+active.length+" 個のキーバインド有効"; }
  ...
  /* ---------- init ---------- */
  renderRows(); regen(); syncDesktop(); doSum(); $("rubyOut").innerHTML=rubyAnnotate($("rubyIn").value);
})();
</script>
</body>
</html>
...
/* Stage the same page as the native app's www/index.html (kept out of git;
 * regenerated at build time). In the desktop app, window.madokey (preload)
 * turns the editor's binds into real global hotkeys. */
const appWww = path.join(IDE, "madokey-app", "www");
fs.mkdirSync(appWww, { recursive: true });
fs.writeFileSync(path.join(appWww, "index.html"), html);
console.log("staged madokey-app/www/index.html");
