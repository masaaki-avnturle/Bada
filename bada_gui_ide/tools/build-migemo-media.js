...
<header>
  ...
  <button class="btn" id="gConnect">☁ Google接続</button>
  <button class="btn" id="cloudBtn">🔗 URL追加</button>
...
<div id="gmodal" style="position:fixed;inset:0;background:#000a;display:none;align-items:center;justify-content:center;z-index:22;padding:14px;">
  <div class="sheet">
    <h3>☁ Google Drive / Photos を接続</h3>
    <p class="note" style="color:var(--dim);font-size:12px;margin:0 0 6px">
      実クラウド連携には<strong>あなた自身の OAuth クライアントID</strong>が必要です
      (Google Cloud Console → 認証情報 → OAuth 2.0 クライアント「ウェブ アプリケーション」)。
      承認済みの JavaScript 生成元に、このアプリを開いている生成元
      (デスクトップ版なら <code>http://127.0.0.1:*</code> / ホスト時は該当 https)を登録してください。
      <code>file://</code> では Google のサインインは動作しません。
    </p>
    <label>OAuth クライアント ID</label>
    <input id="g_client" placeholder="xxxx.apps.googleusercontent.com"/>
    <div class="row">
      <button class="add" id="g_connect">接続してメディアを読み込む</button>
      <button class="cancel" id="g_cancel">閉じる</button>
    </div>
    <div id="g_status" class="note" style="color:var(--dim);font-size:12px;margin-top:8px"></div>
    <div class="row" id="g_actions" style="display:none">
      <button class="btn" id="g_drive">Drive から読み込む</button>
      <button class="btn" id="g_photos">Photos から読み込む</button>
    </div>
  </div>
</div>
...
(function(){
  ...
  /* ==== B: real cloud connect — Google Drive / Photos (OAuth) ==== */
  var GK="migemomedia.gclient";
  var gToken=null;
  function gid(){ try{return localStorage.getItem(GK)||"";}catch(e){return "";} }
  function gsay(s){ $("g_status").innerHTML=s; }
  $("gConnect").addEventListener("click", function(){
    $("g_client").value=gid();
    $("gmodal").style.display="flex";
    if(!/^https?:$/.test(location.protocol)){
      gsay('<span style="color:#e8cf8a">注意: 現在 <code>'+location.protocol+'</code> で開いています。Google サインインには http(s) 生成元が必要です(デスクトップ版、または https ホスト)。手動URL/PC取り込みはこのままご利用いただけます。</span>');
    }
  });
  $("g_cancel").addEventListener("click", function(){ $("gmodal").style.display="none"; });
  ...
  function loadGIS(){
    return new Promise(function(res,rej){
      ...
      s.onload=function(){ setTimeout(res,100); };
      s.onerror=function(){ rej(new Error("GIS スクリプトを読み込めません(オフライン?)")); };
      document.head.appendChild(s);
      setTimeout(function(){ if(!(window.google&&window.google.accounts)) rej(new Error("GIS 読み込みタイムアウト")); }, 8000);
    });
  }
  $("g_connect").addEventListener("click", function(){
    var cid=$("g_client").value.trim();
    if(!cid){ gsay('<span style="color:#f2a49b">クライアントIDを入力してください。</span>'); return; }
    try{ localStorage.setItem(GK,cid); }catch(e){}
    gsay("Google に接続中…");
    loadGIS().then(function(){
      var tc=window.google.accounts.oauth2.initTokenClient({
        ...
        callback: function(resp){
          if(resp&&resp.access_token){ gToken=resp.access_token; gsay('<span style="color:#7ce0a3">接続しました。Drive / Photos から読み込めます。</span>'); $("g_actions").style.display="flex"; }
          else { gsay('<span style="color:#f2a49b">トークンを取得できませんでした。</span>'); }
        }
      });
      tc.requestAccessToken();
    }).catch(function(e){ gsay('<span style="color:#f2a49b">'+esc(e.message)+' 手動URL/PC取り込みをご利用ください。</span>'); });
  });

  function gAddBlob(url, mime, title){
    return fetch(url,{headers:{Authorization:"Bearer "+gToken}}).then(function(r){ if(!r.ok) throw new Error("HTTP "+r.status); return r.blob(); })
      .then(function(b){ var o=URL.createObjectURL(b);
        state.items.push({ id:"m"+Date.now()+"_"+Math.floor(Math.random()*1e5), title:title||"", type:/^video\\//.test(mime)?"video":"image", src:o, source:"gdrive", zoned:false, persist:false });
      });
  }
  $("g_drive").addEventListener("click", function(){
    if(!gToken){ gsay("先に接続してください。"); return; }
    gsay("Drive を読み込み中…");
    var q=encodeURIComponent("(mimeType contains 'image/' or mimeType contains 'video/') and trashed=false");
    fetch("https://www.googleapis.com/drive/v3/files?q="+q+"&pageSize=18&fields=files(id,name,mimeType)",{headers:{Authorization:"Bearer "+gToken}})
      .then(function(r){ return r.json(); })
      .then(function(j){
        var files=(j&&j.files)||[]; if(!files.length){ gsay("Drive に画像/動画が見つかりません。"); return; }
        var proms=files.map(function(f){ return gAddBlob("https://www.googleapis.com/drive/v3/files/"+f.id+"?alt=media", f.mimeType, f.name.replace(/\\.[^.]+$/,"")); });
        Promise.all(proms.map(function(p){return p.catch(function(){});})).then(function(){ save(state); render(); gsay('<span style="color:#7ce0a3">'+files.length+' 件を取り込みました。</span>'); });
      }).catch(function(e){ gsay('<span style="color:#f2a49b">Drive 読込に失敗: '+esc(e.message)+'</span>'); });
  });
  $("g_photos").addEventListener("click", function(){
    if(!gToken){ gsay("先に接続してください。"); return; }
    gsay("Photos を読み込み中…");
    fetch("https://photoslibrary.googleapis.com/v1/mediaItems?pageSize=25",{headers:{Authorization:"Bearer "+gToken}})
      .then(function(r){ return r.json(); })
      .then(function(j){
        var items=(j&&j.mediaItems)||[]; if(!items.length){ gsay("Photos にメディアが見つかりません。"); return; }
        items.forEach(function(mi){
          var isV=/^video/.test(mi.mimeType||"");
          var src=mi.baseUrl+(isV?"=dv":"=w1024-h1024");
          state.items.push({ id:"m"+Date.now()+"_"+Math.floor(Math.random()*1e5), title:mi.filename||"", type:isV?"video":"image", src:src, source:"gphotos", zoned:false, persist:true });
        });
        save(state); render(); gsay('<span style="color:#7ce0a3">'+items.length+' 件を取り込みました。</span>');
      }).catch(function(e){ gsay('<span style="color:#f2a49b">Photos 読込に失敗: '+esc(e.message)+'</span>'); });
  });
...
/* also stage it as the Migemogram Media app's www/index.html (Electron / Cordova) */
const APPWWW = path.join(IDE, "migemomedia-app", "www");
if (fs.existsSync(path.join(IDE, "migemomedia-app"))) {
  fs.mkdirSync(APPWWW, { recursive: true });
  fs.writeFileSync(path.join(APPWWW, "index.html"), html);
  console.log("staged migemomedia-app/www/index.html");
}
