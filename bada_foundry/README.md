...
**主な機能:**
- **要求の説明可能な取り込み** — 「作りたいアプリ」を自然文で入力すると、未知事前予知エンジンが
  要求文を Ξ・幾何で解析し、どの方程式がなぜ選ばれ、どう要求を満たすかを「説明」タブで明示します。
- **方程式群 → 言語変換** — 方程式群を **Bada / JavaScript / Python / Ruby** の実行可能なソースコードへ変換
  （Source タブで言語切替、各言語で動く）。
- **APK/EXE 化プロジェクトの生成** — 生成した子アプリを、そのまま **APK と Windows10/11 EXE に
  ビルドできるプロジェクト一式（.zip）** として出力（www + electron + capacitor + GitHub Actions + README）。
  展開して Actions を回すだけで、生成アプリ自体が APK/EXE になります。
