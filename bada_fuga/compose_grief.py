...
# -*- coding: utf-8 -*-
"""
Requiem BADA V — Dolore (B♭ minor, ♩=36)
  木琴シンセをやめ、ピアノ演奏 (S/A/T/B + 高音の B-A-D-A + 低音) に戻す。
  電子シンセサイザーは不協和音のクラスター (根音 + 短 2 度 + 三全音 + 長 7 度) を持続させる層 (C)。
  ピアノの音符は短く切って間を空け (点描)、その隙間をシンセの不協和な持続と長い残響がつなぐ。
  全体を B♭ 短調へ移調 (ニ短調 −4 半音)。B-A-D-A は G♭-F-B♭-F になる。
"""
import sys, math
from compose import *
import compose
...
def high_bada(P, bar0, nreps, label='B-A-D-A (ピアノ高音)'):
    ...
    for r in range(nreps):
        for i, m in enumerate(mat_):
            add('H', bar0 + r * 4 + i, 0, 4, m, 0.45, label if (i == 0 and r == 0) else None)
            add('H', bar0 + r * 4 + i + 2, 0, 4, m - 5, 0.3)

def build():
    T = Piece(130); compose.build_fugue(T, 0)
    P = Piece(TOTAL)
    P.section(0, 'I. Introitus — Dolore', '嘆きのバスのパッサカリア。ピアノは点描、電子シンセの不協和音が隙間をつなぐ')
    ...
    high_bada(P, 2, 1)
    clusters(P, 0, INTRO, vel=0.22, dense=True)
    d1 = INTRO
    ...
    clusters(P, d1, d1 + 32, vel=0.15)
    for k in range(0, 32, 4): add('H', d1 + k, 0, 4, n('D7'), 0.3)
    r0 = d1 + 32
    P.section(r0, 'Ritornello — Dolore', '嘆きのバスの回帰。B-A-D-A をテノール → アルト、不協和音は濃くなる')
    lament(P, r0, 2, mantra_voices=('T', 'A'), dyn=lambda c: 0.7)
    high_bada(P, r0, 1, label=None)
    clusters(P, r0, r0 + RIT, vel=0.24, dense=True)
    d2 = r0 + RIT
    copy_bars(T, P, SEG2[0], SEG2[1], d2)
    P.sections = [(b, t, s) for b, t, s in P.sections if not (b == d2 and 'Soggetto III' in t)]
    P.section(d2, 'III. Fuga a tre soggetti — Dolore', '三主題の三重結合 ×3 を点描で。最後の休止で途切れる')
    for bar in range(d2, d2 + 26): P.dyn[bar] = 0.85
    low_octaves(P, d2, d2 + 24, every=2, vel=0.45)
    clusters(P, d2, d2 + 24, vel=0.17)
    for k in range(0, 24, 4): add('H', d2 + k, 0, 4, n('D7'), 0.3)
    c0 = d2 + 26
    P.section(c0, 'IV. Lacrimosa — Passacaglia', '嘆きのバス ×4。B-A-D-A が声部を渡り、不協和音の持続だけが残って消える')
    lament(P, c0, 4, mantra_voices=('A', 'S', 'T', 'A'), dyn=lambda c: max(0.2, 0.75 - 0.15 * c))
    high_bada(P, c0, 2, label=None)
    clusters(P, c0, c0 + CODA, vel=0.26, dense=True)
    ...
    add('H', TOTAL - 2, 0, 8, n('D7'), 0.4); add('H', TOTAL - 1, 0, 4, n('A6'), 0.25)
    for i, m in enumerate([n('D4'), n('Eb4'), n('Ab4'), n('C#5')]): add('C', TOTAL - 2, 0, 9, m, 0.2 * (1.0 if i == 0 else 0.7))
    P.dyn[TOTAL - 2] = P.dyn[TOTAL - 1] = 0.25
    return P

META = {
    'style': 'grief', 'detach': 0.5,
    'title': 'Requiem BADA V — Dolore',
    'subtitle': 'B♭ minor · ♩=36 ／ ピアノの点描 (音符の間を空ける) を電子シンセの不協和音と残響がつなぐ、嘆きのバスのフーガ=レクイエム',
    'footer': ['主題 I ← MOTHER (LUNA SEA) ／ 主題 II ← トラック18 ／ 主題 III ← B♭-A-D-A (BADA, B♭短調では G♭-F-B♭-F) ／ 嘆きのバス ← 半音下行',
               'エピソード ← LOVELESS ／ トラック17 ／ トラック8 のため息 ／ 録音 090146    ｜  合成: グランドピアノ (S/A/T/B/H/L) · 電子シンセ不協和音クラスター (C)',
               'ピアノは各音符を半分の長さで切って間を空け、シンセの 根音+短2度+三全音+長7度 の持続と 4.5 秒の残響が音符同士をつなぐ。'],
}

if __name__ == '__main__':
    out = sys.argv[1] if len(sys.argv) > 1 else 'score_grief.json'
