...
# -*- coding: utf-8 -*-
"""
Requiem BADA — Tablet Sessions XVI · Sintetizzatore reale (主題に重なる音を、11:21 の録音のシンセサイザーの実音に)
  XV のバイオリンを、2026-09-24 の 11:21 (ピアノとシンセサイザーを重ねた録音) から切り出したシンセの持続音の実音
  (build_synthbank.py: ピアノの打鍵が減衰したあとに残る、一定に伸びる音 = シンセ) に置き換えた。主題はピアノ (録音の 1 音) のまま、
  フーガのまま、レクイエム。サンプラー (rid 'VOXSY') はこの実音だけから選び、減衰させずにループで伸ばす。
  以下は XV の説明:
  ...
  使い方: python compose_tablet16.py <bank.json> [score_tablet12.json]
"""
import sys
from compose import *
import compose
...
def build():
    ...
    # ---------------- Fuga I (ホ短調): バッハ風
    f = b
    P.section(b, 'Fuga I — %s のバッハ風フーガ (ホ短調)' % hm(E2), '提示 → エピソード → 下属調の入り → ストレッタ → 保続低音 — 主題の入りに 11:21 のシンセの実音が同じ音で重なる')
    n_ = T11.bach_fugue(P, b, E2, '②')
    T2.MANTRA.append((f, f + n_, E1, ('PK',))); PADS.append((f, f + n_, 0.055)); LEADS.append((f, f + n_, 0.38))
    ...
    # ---------------- Fuga II (変ロ短調): 二重フーガ
    f = b; entries, labmap = [], {}
    P.section(b, 'Fuga II — 主題 ①③ 〈%s · %s〉 (変ロ短調)' % (hm(E1), hm(RB)), '11:23 と 11:18 の主題の二重フーガ — 11:21 のシンセの実音が主題をなぞる')
    ...
    T2.MANTRA.append((f, f + 10, RB, ('PK',))); PADS.append((f, f + 10, 0.055)); LEADS.append((f, f + 10, 0.38))
    ...
    f = b; entries, labmap = [], {}
    P.section(b, 'Finale — Stretto a cinque soggetti (ヘ短調)', '5 本の録音の主題が次々と入り、最後の 2 つはストレッタで重なる → 保続低音 — 11:21 のシンセの実音が全部の主題をなぞる')
    ...
    T2.MANTRA.append((f, f + 10, RF, ('PK',))); PADS.append((f, f + 10, 0.06)); LEADS.append((f, f + 10, 0.4))
    ...
    P.section(b, 'In paradisum — %s の本当の終わり' % hm(SY), 'ピアノとシンセサイザーの録音 11:21 の最後の 20 秒を実音のまま → ヘ長調の和音を、ピアノと 11:21 のシンセの実音が静かに鳴らして閉じる')
    ...
    for k, (m, pan) in enumerate(((57, -0.2), (66, 0.2))):                       # 11:21 のシンセの実音で和音 (ヘ長調で C4・A4)
        add('SP', b * BPB + 1.0, 9.0, m, 0.11, None, rid='VOXSY', pan=pan)
...
META = {
    ...
    'title': 'Requiem BADA — Tablet Sessions XVI · Sintetizzatore reale',
    'subtitle': '主題はピアノ、それに重なるのは 11:21 の録音のシンセサイザーの実音 — フーガのまま、レクイエム (♩=60)',
    ...
    'footer': ['Introitus: 11:23 → 08:53 (ホ短調) → Fuga I (08:53, バッハ風) → Lacrimosa 11:18 (変ロ短調) → Fuga II (二重) → Sanctus: 08:49 → 11:21 (ヘ短調) → Finale (ストレッタ) → 11:21 の終わり → ヘ長調',
               'シンセの実音: 11:21 の録音からピアノの打鍵が減衰したあとに残る持続音 (C♯2〜G♯3 の 8 音) を切り出し、主題の高さに移して減衰させずに伸ばす。'],
}

if __name__ == '__main__':
    out = sys.argv[2] if len(sys.argv) > 2 else 'score_tablet16.json'
