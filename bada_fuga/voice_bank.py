...
"""取り出した歌声 (extract_voice.py) から、歌声の 1 音のサンプル集を作る (synth.py の sampler_tone で 4 声を歌声で鳴らすため)。
  ...
  使い方: python voice_bank.py <voice.json> <出力フォルダ> [録音 id ...]
...
def main(vjson, out, rids):
    ...
    for rid, r in V.items():
        ...
        for key, (d, rms, midi, clip) in sorted(best.items()):
            ...
            bank['samples'].append({'rid': 'VOX', 'from': rid, 'midi': round(midi, 3), 'dur': round(len(clip) / sr, 3), 'purity': 1.0, 'file': fn})
