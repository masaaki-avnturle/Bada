...
# -*- coding: utf-8 -*-
"""
Requiem BADA — Tablet Sessions XXI · Pastorale pianistica (XX の全音を、実録音のピアノの音に書き換えた版)
  XX (出だしの伴奏なし、フーガの 4 声だけ) の全音を、9/23・9/24 の録音から切り出したピアノの 1 音 (自然に減衰) に置き換えた。
  4 声の音は、その主題の元の録音のピアノの音 (色で示す)。以下は XX の説明:
  ...
  使い方: python compose_tablet21.py <bank.json (9/23・9/24 の採譜と 1 音)> [score_tablet19.json]
"""
import sys
from compose import *
import compose
...
def build():
    ...
    # ---------------- Fuga I (ト短調)
    f = b
    P.section(b, 'Pastorale — Fuga I 〈%s〉 (ト短調)' % hm(E2), '伴奏なしで、いきなりフーガから: 08:53 の主題 (跳躍を畳み、長短の揺れをつけた) のバッハ風フーガ — 4 声とも実録音のピアノ')
...
META = {
    ...
    'title': 'Requiem BADA — Tablet Sessions XXI · Pastorale pianistica',
    'subtitle': '全音を実録音のピアノに — 伴奏なし、フーガの 4 声だけのレクイエム (ト短調 / ト長調, ♩=66)',
    'legend': ['PF'], 'vname': {'PF': 'ピアノ (終わりの和音)'},
    'footer': ['Fuga I (08:53) → Kyrie: Fuga II (11:23 · 11:18) → Gloria (ト長調, 08:09) → Misericordia: Fuga III (9/23 08:06 · 08:49) → Finale (7 つの主題) → Wiegenlied (ト長調)',
               '音はすべて 9/23・9/24 の録音から切り出したピアノの 1 音 (自然に減衰)。伴奏はなく 4 声のフーガだけ。主題は跳躍を畳み、シチリアーナの揺れ。'],
}

if __name__ == '__main__':
    out = sys.argv[2] if len(sys.argv) > 2 else 'score_tablet21.json'
