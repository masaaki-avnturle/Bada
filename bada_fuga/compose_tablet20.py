...
# -*- coding: utf-8 -*-
"""
Requiem BADA — Tablet Sessions XX · Pastorale sintetica (出だしの伴奏を消し、全体を 11:21 のシンセサイザーの実音のレクイエムに — 旋律はフーガ)
  XIX から、始めの出だしの伴奏 (羊飼いの笛のドローンの 4 小節と、その後のドローン) を消し、4 声すべてを 2026-09-24 11:21 の録音から
  切り出したシンセサイザーの持続音の実音 (rid 'VOXSY'、減衰させずループで伸ばす) にした。伴奏はなく、フーガの 4 声だけが曲になる。
  主題・調・形は XIX のまま (跳躍を畳んだなだらかな主題、シチリアーナの揺れ、ト短調とト長調)。
  以下は XIX の説明:
  ...
  使い方: python compose_tablet20.py <bank.json (9/23・9/24 の採譜と 1 音)> [score_tablet19.json]
"""
import sys
from compose import *
import compose
...
def build():
    ...
    total = 16 + 1 + 10 + 8 + 1 + 10 + 1 + 14 + 8 + 2
    P = Piece(total)
    for k in range(total): P.tempo[k] = BPM; P.dyn[k] = 0.85
    b = 0
    # ---------------- Fuga I (ト短調)
    f = b
    P.section(b, 'Pastorale — Fuga I 〈%s〉 (ト短調)' % hm(E2), '伴奏なしで、いきなりフーガから: 08:53 の主題 (跳躍を畳み、長短の揺れをつけた) のバッハ風フーガ — 4 声とも 11:21 のシンセの実音')
    ...
    # ---------------- Gloria — ト長調の牧歌
    g0 = b
    P.section(b, 'Gloria — ト長調の牧歌 〈%s〉' % hm(E3), '生誕の光: ト長調で、9/23 08:09 の主題を長調にして上声が歌い、下の 3 声がやさしく支える')
    ...
    # ---------------- Wiegenlied (ト長調): 子守歌
    w0 = b
    P.section(b, 'Wiegenlied — 子守歌 (ト長調)', '飼い葉桶の子守歌: ト長調で 11:23 の主題を長調にして pp で歌い、静かに消えていく')
    ...
    for k, (m, g) in enumerate(((38, 0.2), (45, 0.14), (54, 0.11), (57, 0.1), (62, 0.09))): add('SP', b * BPB + 0.35 * k, 7.0, m, g, None, rid=SYN, pan=(-0.3, 0.3, -0.15, 0.15, 0.0)[k])
...
META = {
    ...
    'title': 'Requiem BADA — Tablet Sessions XX · Pastorale sintetica',
    'subtitle': '出だしの伴奏を消し、全体を 11:21 のシンセサイザーの実音のレクイエムに — 旋律はフーガ (ト短調 / ト長調, ♩=66)',
    'legend': ['SP'], 'vname': {'SP': '11:21 のシンセ (終わりの和音)'},
    'footer': ['Fuga I (08:53) → Kyrie: Fuga II (11:23 · 11:18) → Gloria (ト長調, 08:09) → Misericordia: Fuga III (9/23 08:06 · 08:49) → Finale (7 つの主題) → Wiegenlied (ト長調)',
               '音はすべて 11:21 の録音から切り出したシンセの持続音 (実音) を移調したもの。伴奏はなく 4 声のフーガだけ。主題は跳躍を畳み、シチリアーナの揺れ。'],
}

if __name__ == '__main__':
    out = sys.argv[2] if len(sys.argv) > 2 else 'score_tablet20.json'
