...
# -*- coding: utf-8 -*-
"""
Requiem BADA — Tablet Sessions XII · Risonanza (実音のピアノと、本格的なシンセサイザーが共鳴するレクイエムとフーガ)
  ...
  使い方: python compose_tablet12.py <bank.json> [score_tablet12.json]
"""
import sys
from compose import *
import compose
...
def post(P, events, extras):
    ...
    for b0, b1, g in PADS:                                               # 共鳴するパッド: 2 小節ごと、うねりは 2 小節で 1 往復
        for bar in range(b0, b1, 2):
            c = chord(P.harm[bar * BPB] or 'Dm'); nb = min(2, b1 - bar)
            for k, (pc, tg, pan) in enumerate(((c['root'], 45, 0.0), (c['fifth'], 52, -0.45), (c['third'], 57, 0.45))):
                add('RS', bar * BPB, nb * BPB + 0.5, near(pc, tg), g * (1.0 if k == 0 else 0.7), None, lfo=0.125, phase=0.0, pan=pan)
    for b0, b1, g in LEADS:                                              # 主題の入り (ラベル付きの音) に共鳴するリード
        for v in ('S', 'A', 'T'):
            for s, d, m, lab in events[v]:
                if lab and b0 * BPB <= s < b1 * BPB: add('RL', s, d * 0.92, m, g, None, pan=(-0.2 if v == 'S' else 0.2))
...
def build():
    ...
    b = 0
    P.section(b, 'Introitus — Mix 〈%s → %s〉' % (hm(E1), hm(E2)), 'ホ短調 ／ 実音のピアノ 2 本をつなぐ — その下で共鳴するシンセのパッドがゆっくりうねる')
    b += mix(P, b, E1, E2, 2)
    T2.MANTRA.append((0, b, E1, ('PK',))); PADS.append((0, b, 0.04))
    # ---------------- Fuga I (ホ短調): バッハ風
    f = b
    P.section(b, 'Fuga I — %s のバッハ風フーガ (ホ短調)' % hm(E2), '提示 → エピソード → 下属調の入り → ストレッタ → 保続低音 — 主題の入りに共鳴するシンセが重なり、パッドがうねる')
    ...
    pivot(P, b, E2, -4); b += 1
    # ---------------- Lacrimosa (変ロ短調)
    P.section(b, 'Lacrimosa — %s の実音 (変ロ短調)' % hm(RB), 'ループせずそのまま 40 秒 — 共鳴するパッドが下で支える')
    ...
    # ---------------- Fuga II (変ロ短調): 二重フーガ
    f = b; entries, labmap = [], {}
    P.section(b, 'Fuga II — 主題 ①③ 〈%s · %s〉 (変ロ短調)' % (hm(E1), hm(RB)), '11:23 と 11:18 の主題の二重フーガ — 共鳴するシンセが主題を響かせる')
    ...
    s0 = b
    P.section(b, 'Sanctus — Mix 〈%s → %s (ピアノとシンセ)〉 (ヘ短調)' % (hm(RF), hm(SY)), '08:49 から、ピアノとシンセサイザーの録音 11:21 へ実音のまま — 録音のシンセに共鳴するパッドがそっと寄り添う')
    b += mix(P, b, RF, SY, 3)
    T2.MANTRA.append((s0, b, RF, ('PK',))); PADS.append((s0, s0 + 8, 0.04)); PADS.append((s0 + 8, b, 0.025))
    # ---------------- Finale (ヘ短調): 5 つの主題のストレッタ → 保続低音
    f = b; entries, labmap = [], {}
    P.section(b, 'Finale — Stretto a cinque soggetti (ヘ短調)', '5 本の録音の主題が次々と入り、最後の 2 つはストレッタで重なる → 保続低音 — 共鳴するシンセが全部の主題を響かせる')
    ...
    P.section(b, 'In paradisum — %s の本当の終わり' % hm(SY), 'ピアノとシンセサイザーの録音 11:21 の最後の 20 秒を実音のまま → ヘ長調の和音を共鳴シンセがゆっくり開いて、閉じていく')
    ...
    for k, (pc, tg, pan) in enumerate(((2, 38, 0.0), (9, 45, -0.5), (6, 54, 0.5), (2, 62, 0.0))):
        add('RS', b * BPB + 0.5, 10.0, near(pc, tg), 0.08 * (1.0 if k < 2 else 0.6), None, lfo=0.05, phase=0.0, pan=pan)
...
META = {
    ...
    'title': 'Requiem BADA — Tablet Sessions XII · Risonanza',
    'subtitle': '実音のピアノと、本格的なシンセサイザーが共鳴するレクイエムとフーガ — 9/24 の録音 5 本で (♩=60)',
    'legend': ['TB', 'RS', 'RL', 'PK'],
    'footer': ['Introitus: 11:23 → 08:53 (ホ短調) → Fuga I (08:53, バッハ風) → Lacrimosa 11:18 (変ロ短調) → Fuga II (二重) → Sanctus: 08:49 → 11:21 (ヘ短調) → Finale (ストレッタ) → 11:21 の終わり → ヘ長調',
               'シンセ: デチューンした鋸歯波 3 本 + サブを共鳴する 2 次ローパスに通し、カットオフが LFO で往復。パッドは和音を、リードは主題の入りを共鳴させる。'],
}

if __name__ == '__main__':
    out = sys.argv[2] if len(sys.argv) > 2 else 'score_tablet12.json'
