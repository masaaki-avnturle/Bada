...
def main(score='score.json', wav='fuga.wav', out='fuga.mp4'):
    ...
    if meta.get('style') == 'recsampler':
        COL.update({'TB': (110, 210, 250), 'OS': (200, 200, 215), 'DN': (130, 100, 160), 'PK': (235, 70, 90), 'PF': (220, 220, 230),
                    'BV': (250, 170, 200), 'LD': (255, 230, 120), 'PD': (120, 110, 170), 'SB': (90, 200, 255), 'GT': (240, 120, 60), 'DR': (235, 70, 90), 'GL': (200, 235, 255), 'SUB': (110, 90, 200), 'RS': (120, 150, 255), 'RL': (255, 200, 90), 'VN': (230, 150, 110)})
        VNAME.update({'GL': 'シンセ (高音)', 'SUB': 'シンセ (低音)', 'RS': '共鳴シンセ (パッド)', 'RL': '共鳴シンセ (主題)', 'VN': 'バイオリン (主題に重なる)'})
    ...
    SRC = [n.get('rid') or n.get('src', '') if n['v'] in ('S', 'A', 'T', 'B', 'OS', 'PF') else '' for n in notes]
