"""
Requiem BADA — Tablet Sessions XXXV · Kimigayo — Acceptance (BADA 528 Acceptance と、レクイエム・讃美歌・フーガの融合)
  XXXIV (君が代のレクイエムと讃美歌の哀しみのフーガ) に、以前の「BADA 528 — Acceptance」(坂本龍一『リトル・ブッダ』の Acceptance を
  思わせる哀愁の弦楽、コントラバス独奏、タンプーラ、バンスリ、タブレット録音の主題 T) を融合した。
    - Prologo: コントラバス独奏 (B♭1 → F2 B♭2 → D♭3 C3 A♭2)、タンプーラの B♭–F
    - 讃美歌とフーガ (XXXIV): 4 声は録音から切り出したピアノの実音、厚い弦 (Vn I・Vn II・Va・Vc) が声部に重なる
    - Acceptance (ホ短調へ): 2026-09-22 090146 の録音から作った主題 T を厚い弦がオクターヴで歌い、ピアノの分散和音、2 回目はバンスリ
      → 録音 090146 の実音 (ホ短調のまま) にコントラバスとタンプーラが寄り添う → 変ロ短調へ戻る
    - Finale: 君が代の全旋律の讃美歌を厚い弦とバンスリで、アーメン終止、ユニゾンの B♭ → Coda 08:06 の本当の終わり、タンプーラが消える
  提出された録音: 9/22 090146・090933 (ピアノの 1 音と実音)、9/23 08:06、9/24 11:18 (実音の抜粋)。
  使い方: python compose_tablet35.py <bank35.json (bank17 + 9/22 の 2 本)> [score_tablet35.json]
"""
import sys, json
from compose import *
import compose
...
T1, T2R = '20260922_090146', '20260922_090933'
KEYS.update({T1: 2, T2R: 2}); T7.KEYS.update(KEYS)
...
SUBG = {}                      # 小節 → サブベースの音量
STR, TAN, BNS, VOX1 = [], [], [], []   # 弦の重ね (b0,b1,gain) / タンプーラ (b0,b1,gain) / バンスリ (b0,b1,gain) / Vn I のオクターヴ上 (b0,b1,gain)
...
def post(P, events, extras):
    ...
    for b0, b1, gain in STR:                                                 # 厚い弦が 4 声に重なる (Acceptance の弦)
        for v, iv in (('S', 'V1'), ('A', 'V2'), ('T', 'VA'), ('B', 'VC')):
            for s_, d, m, lab in events[v]:
                if b0 * BPB <= s_ < b1 * BPB and m is not None: add(iv, s_, d * 1.05, m, gain * (1.1 if v == 'S' else 1.0), None)
    for b0, b1, gain in VOX1:                                                # Vn I がソプラノの 1 オクターヴ上で
        for s_, d, m, lab in events['S']:
            if b0 * BPB <= s_ < b1 * BPB and m is not None: add('V1', s_, d * 1.05, m + 12, gain, None)
    for b0, b1, gain in BNS:                                                 # バンスリがソプラノの 1 オクターヴ上で
        for s_, d, m, lab in events['S']:
            if b0 * BPB <= s_ < b1 * BPB and m is not None and lab: add('BN', s_, d, m + 12, gain, None)
    for b0, b1, gain in TAN:                                                 # タンプーラ (Pa Sa Sa Sa)
        for bar in range(b0, b1):
            for k, m in enumerate((45, 50, 50, 38)): add('TA', bar * BPB + k, 3.5, m, gain * (0.8 if k == 3 else 1.0), None)
...
def build():
    ...
    VS9 = {'S': T1, 'A': T2R, 'T': T1, 'B': T2R}                                          # Acceptance の区間は 9/22 のピアノの音
    total = 3 + 6 + 6 + 16 + 16 + 8 + 6 + 10 + 12 + 5 + 3
    P = Piece(total)
    ...
    # ---------------- Inno I
    f = b; E = []
    P.section(b, 'Inno I — 讃美歌 「君が代は」「千代に八千代に」 (変ロ短調)', '4 声のコラール (ピアノの実音) に厚い弦が重なる — 掛留とアーメン終止')
    ...
    for k in range(6): P.dyn[f + k] = 1.15; SUBG[f + k] = 0.05
    STR.append((f, f + 6, 0.13)); BELL.append((f, f + 6, 0.09, B1))
    T2.MANTRA.append((f, f + 6, B1, ('PK',))); CT.LAYOUT.append((f, f + 6, SEMIS, VOICE_SRC, {})); b = f + 6
    # ---------------- Fuga I
    f = b
    P.section(b, 'Fuga I — 「君が代は」のバッハ風フーガ (変ロ短調)', 'ピアノ独りで始まり、弦が加わる (Fugato のように) — 提示 → エピソード → 下属調 → ストレッタ → 保続低音')
    n_ = T11.bach_fugue(P, b, 'KIMI', '①')
    for k in range(n_): P.dyn[f + k] = 1.25; SUBG[f + k] = 0.045
    STR.append((f + 8, f + n_, 0.12))
    T2.MANTRA.append((f, f + n_, B1, ('PK',))); CT.LAYOUT.append((f, f + n_, SEMIS, VOICE_SRC, {})); b = f + n_
    # ---------------- Acceptance — 主題 T (ホ短調)
    f = b
    P.section(b, 'Acceptance — 受容 (ホ短調)', '9/22 090146 の録音から作った主題 T を厚い弦がオクターヴで歌い、ピアノ (9/22 の実音) が分散和音を降らせる。2 回目はバンスリが 1 オクターヴ上で重なる')
    ...
    for k in range(10): P.dyn[f + k] = 1.25; SUBG[f + k] = 0.045
    STR.append((f + 4, f + 10, 0.13))
    T2.MANTRA.append((f, f + 10, B2, ('PK',))); CT.LAYOUT.append((f, f + 10, SEMIS, VOICE_SRC, {'主題 ④ (08:06)': B1})); b = f + 10
    # ---------------- Finale
    f = b; E = []
    P.section(b, 'Finale — 全旋律の讃美歌、厚い弦とバンスリ (変ロ短調)', '君が代の全旋律を讃美歌で: 厚い弦がオクターヴで、バンスリが「千代に八千代に」に重なり、「君が代は」のストレッタ — アーメン、ユニゾンの B♭')
    t = 0
...
if __name__ == '__main__':
    out = sys.argv[2] if len(sys.argv) > 2 else 'score_tablet35.json'
