"""
Requiem BADA — Tablet Sessions XXXII · Kimigayo — Fuga e Requiem del basso profondo (重低音のシンセと実音のピアノ)
  全体を「重低音のシンセサイザー」と「実音のピアノ」(9/23・9/24 の録音から切り出した 1 音) だけで:
    - 重低音: 和声の根音を B♭0・B♭1 あたりの正弦波のサブベース (synth.py sub_tone) が半小節ごとに支え、ゆっくり立ち上がりゆっくり消える
    - メロディーはフーガに: 君が代の各句を主題にしたフーガが 3 つ (第 1 句のバッハ風フーガ、第 2 句のフーガ、第 1 句と「怒りの日」の二重フーガ)
    - 主題はレクイエムに: 各句を 2 倍の長さのコラール (4 声、全音符) として、フーガの間に置く (Requiem I〜III)
    - Finale: 全旋律をソプラノに、その下で第 1 句がストレッタで重なり、最後は全声部がユニゾンの B♭、サブベースだけが残って消える
  実音の抜粋と 11:21 のシンセは使わない。変ロ短調、♩=50、葬送の太鼓 (ピアノの低い打鍵) と弔鐘。
  使い方: python compose_tablet32.py <bank17.json> [score_tablet32.json]
"""
import sys, json
from compose import *
import compose
...
def build():
    ...
    # ---------------- Requiem I
    f = b; E = []
    P.section(b, 'Requiem I — 「君が代は」のコラール (変ロ短調)', '第 1 句を 2 倍の長さで、4 声の全音符のコラール — 重低音のサブベースが和声の根音を支え、弔鐘と葬送の太鼓')
    t = chorale(P, f, [KIMI[0]], E, ['君が代は (コラール)'])
    ...
    f = b; E = []
    P.section(b, 'Fuga III — 二重フーガ 〈君が代は · 怒りの日〉', '「怒りの日」をバスが 2 倍の長さで唱え、その上で「君が代は」が A → S → T と入り、ストレッタで重なる — サブベースと弔鐘が強く')
    ...
    # ---------------- Finale — 全旋律 + ストレッタ
    f = b; E = []
    P.section(b, 'Finale — 全旋律とストレッタ (変ロ短調)', '全旋律をソプラノに、その下で「君が代は」が A・T・B にストレッタで重なる — 最後は全声部がユニゾンの B♭')
    t = 0
...
if __name__ == '__main__':
    out = sys.argv[2] if len(sys.argv) > 2 else 'score_tablet32.json'
