...
# -*- coding: utf-8 -*-
"""
Requiem BADA — Tablet Sessions XIII · Risonanza senza onda (共鳴するうねりを消した版)
  ...
  使い方: python compose_tablet13.py <bank.json> [score_tablet12.json]
"""
import sys
from compose import *
import compose
    ...
    for b0, b1, g in LEADS:                                              # 主題の入り (ラベル付きの音) に共鳴するリード
        for v in ('S', 'A', 'T'):
            for s, d, m, lab in events[v]:
                if lab and b0 * BPB <= s < b1 * BPB: add('RL', s, d * 0.92, m, g, None, pan=(-0.2 if v == 'S' else 0.2), lfo=0.0, phase=3.14159, open=0.03)
...
def build():
    ...
    # ---------------- Fuga I (ホ短調): バッハ風
    f = b
    P.section(b, 'Fuga I — %s のバッハ風フーガ (ホ短調)' % hm(E2), '提示 → エピソード → 下属調の入り → ストレッタ → 保続低音 — 主題の入りに共鳴するシンセが同じ音で重なる (うねりなし)')
    ...
    P.section(b, 'In paradisum — %s の本当の終わり' % hm(SY), 'ピアノとシンセサイザーの録音 11:21 の最後の 20 秒を実音のまま → ヘ長調の和音をシンセが一度だけゆっくり開いて、閉じる')
    ...
    for k, (pc, tg, pan) in enumerate(((2, 38, 0.0), (9, 45, -0.5), (6, 54, 0.5), (2, 62, 0.0))):
        add('RS', b * BPB + 0.5, 10.0, near(pc, tg), 0.07 * (1.0 if k < 2 else 0.6), None, lfo=0.0, phase=3.14159, open=3.0, pan=pan)
...
META = {
    ...
    'title': 'Requiem BADA — Tablet Sessions XIII · Risonanza senza onda',
    'subtitle': '共鳴するうねりを消した — 実音のピアノに、主題へ共鳴するシンセだけを重ねたレクイエムとフーガ (♩=60)',
    'legend': ['TB', 'RL', 'PK'],
    'footer': ['Introitus: 11:23 → 08:53 (ホ短調) → Fuga I (08:53, バッハ風) → Lacrimosa 11:18 (変ロ短調) → Fuga II (二重) → Sanctus: 08:49 → 11:21 (ヘ短調) → Finale (ストレッタ) → 11:21 の終わり → ヘ長調',
               'シンセ: デチューンした鋸歯波 3 本 + サブを共鳴する 2 次ローパスに通す。うねる (LFO の) パッドはなし、主題の入りに重なるシンセのフィルターは固定。'],
}

if __name__ == '__main__':
    out = sys.argv[2] if len(sys.argv) > 2 else 'score_tablet13.json'
