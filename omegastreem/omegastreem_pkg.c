/*TCP クライアント）、および関数オブジェクト（`func(<num>)` で生成、生成物に `.new` メソッドと `.data` プロパティを持たせ、`puts(<ident>)` がそのオブジェクトの出力を行う）を備えた最小限で安定した `pkginstallgen.c` です。

使い方
1. ファイルとして保存：pkginstallgen.c
2. ビルド：gcc -o pkginstallgen pkginstallgen.c
3. 実行：./pkginstallgen
4. 生成ディレクトリをビルド：make -C omega_pkg all
5. 実行例：./omega_pkg/omega-bin omega_pkg/examples/objects.omega

注意
- 実装は「簡易言語実行環境（インタープリタ）」を生成するジェネレータです。言語パーサは非常に単純化しており、実用的な完全パーサではありませんが、例示された構文（array/list/hash/func, let/array/list/hash 定義, puts(NAME);, 組み込み関数呼び出し）を処理します。
- 正規表現は POSIX regex（<regex.h>）を使用します。
- ネットワークは IPv4 TCP の簡易クライアント（connect/send/recv, close）を実装します。
- プロセス実行は system() を使用します（戻りコードを返す）。
- 関数オブジェクトは内部で数値 data を保持し、`puts(F)` は data を出力します。`F.new` を呼ぶ簡易構文 `F.new();` をサポート（ここでは新規オブジェクトを複製して格納する動作）。
- セキュリティや完全性は考慮していません。実行環境で自己責任で使用してください。
...
*/
													      ...
													      // pkginstallgen.c
													      // Generates omega_pkg with a runtime that includes:
													      // - basic parsing for let/array/list/hash/func constructs
													      // - puts(NAME); to show objects in requested formats
													      // - builtins: file I/O (read_file, write_file), regex_match, exec_cmd, now, net_connect/send/recv
													      // - func(num) creates a function-object with .data (num) and .new() (clone)
...
static void ensure_dir_recursive(const char *path) {
  ...
  if (len == 0 || len >= sizeof(tmp)) die("path too long");
  strncpy(tmp, path, sizeof(tmp)); tmp[sizeof(tmp)-1]=0;
  if (tmp[len-1]=='/'||tmp[len-1]=='\\') tmp[len-1]=0;
  for (char *p=tmp+1; *p; ++p) {
    if (*p=='/'||*p=='\\') { *p=0; if (MKDIR(tmp)!=0 && errno!=EEXIST) die("mkdir %s: %s", tmp, strerror(errno)); *p='/'; }
  }
  if (MKDIR(tmp)!=0 && errno!=EEXIST) die("mkdir %s: %s", tmp, strerror(errno));
}
static void write_lines_file(const char *path, const char **lines, size_t nlines, int make_exec) {
...
#if !defined(_WIN32) && !defined(_WIN64)
  if (make_exec) if (chmod(path, 0755)!=0) die("chmod %s: %s", path, strerror(errno));
#endif
}

int main(void) {
  puts("pkginstallgen: generating omega_pkg...");
  const char *root = "omega_pkg";
  const char *subdirs[] = {"bin","include","src","examples","etc"};
  ensure_dir_recursive(root);
  for (size_t i=0;i<sizeof(subdirs)/sizeof(subdirs[0]);++i) {
    char p[512]; snprintf(p, sizeof(p), "%s/%s", root, subdirs[i]); ensure_dir_recursive(p);
  }
  ...
  write_lines_file("omega_pkg/include/omega.h", hdr, sizeof(hdr)/sizeof(hdr[0]), 0);
  ...
  write_lines_file("omega_pkg/src/runtime.c", runtime, sizeof(runtime)/sizeof(runtime[0]), 0);
  ...
  write_lines_file("omega_pkg/src/parser.c", parser, sizeof(parser)/sizeof(parser[0]), 0);
  ...
  write_lines_file("omega_pkg/src/omega_bin.c", omega_bin, sizeof(omega_bin)/sizeof(omega_bin[0]), 0);

  const char *compiler[] = {
    "/* omega_compiler.c - simple embedder */",
    "#include <stdio.h>",
    "#include <stdlib.h>",
    "#include <string.h>",
    "static char *read_file_all(const char *p){ FILE *f=fopen(p,\"rb\"); if(!f) return NULL; if(fseek(f,0,SEEK_END)!=0){ fclose(f); return NULL; } long s=ftell(f); if(s<0){ fclose(f); return NULL; } rewind(f); char *b=malloc((size_t)s+1); if(!b){ fclose(f); return NULL; } size_t r=fread(b,1,(size_t)s,f); b[r]='\\0'; fclose(f); return b; }",
  ...
  write_lines_file("omega_pkg/src/omega_compiler.c", compiler, sizeof(compiler)/sizeof(compiler[0]), 0);
  ...
  write_lines_file("omega_pkg/examples/objects.omega", example, sizeof(example)/sizeof(example[0]), 0);
  ...
  const char *mk[] = {
	...
	write_lines_file("omega_pkg/Makefile", mk, sizeof(mk)/sizeof(mk[0]), 0);
	...
	write_lines_file("omega_pkg/bin/omega", launcher, sizeof(launcher)/sizeof(launcher[0]), 1);

	puts("pkginstallgen: finished. Run 'make -C omega_pkg all' then execute the interpreter (omega_pkg/omega-bin).");
	return 0;
  }

/*
これで前回の途中切れ部分からファイル生成処理が完結します。保存後に再度ビルド（gcc -o pkginstallgen pkginstallgen.c）、実行（./pkginstallgen）、および make（make -C omega_pkg all）を行ってください。
*/
