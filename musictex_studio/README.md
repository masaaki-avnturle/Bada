...
- **⚛ 量子作曲 — 擬似量子コンピュータ・シミュレーター × Bada 言語** — Bada VM Pro の
  擬似量子コンピュータ (状態ベクトル実行系・最大8量子ビット) と互換の量子 Bada 言語で作曲:
  - 命令: `qubit q0 q1 ...` / `H` / `X` / `Z` / `CNOT` / `measure` / `state` / `let` / `print`
    (Bada VM Pro と同一) + **音楽拡張** `note [e|q|h|w]`・`chord [e|q|h|w]`・`rest [e|q|h|w]`
  - `note` は全量子ビットを測定し、値を**現在の調の長音階 2 オクターブ**に写像して音符に。
    `chord` は測定値から**五行コード線** (root = v mod 12, 型 = v÷12 mod 5) の和音に
  - 重ね合わせ (`H`) と量子もつれ (`CNOT`) の確率が曲を決め、測定のたびに状態が収縮。
    生成された曲はそのまま譜面化・モーツァルト・ビジョン・シンセ演奏・
    「置き換え/重ねる」取り込みへ流れます
  - インスペクタの ⚛ パネルにベル状態のサンプルプログラムを同梱
