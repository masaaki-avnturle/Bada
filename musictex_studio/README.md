...
- **🎻 オーケストラ多段譜 + コード進行バーコード + 縦の演奏ライン**:
  - **多段総譜**: ヴァイオリン・フルート・トランペット・チェロの4パートを段で重ねて表示
    (パート名・段をまたぐ小節線・左ブラケット・拍位置揃え)。**パートタブ**で各段の
    MusixTeX を個別編集、ライブラリで段ごとの楽器 (音色) を切替
  - **バーコード音階譜**: 譜面上部に各小節のコード進行を表示 — コードの構成音を
    12音階の位置に置いた波長色の縦線 (バーコード状)。ルート音は太線、コード名付き
  - **縦の演奏ライン**: 譜面をクリックすると赤い縦ラインがその位置 (0.5拍単位) へ移動。
    ▶ でそこから**全パートを各楽器の音色で同時演奏**し、ラインが譜面上を移動
    (バーコード・スペクトル帯も同期発光、自動横スクロール)
  - `.tex 保存` は **MusixTeX の多段オーケストラ譜** (`\instrumentnumber` + `&` 区切り・
    楽器名付き) で書き出し — texlive-music でそのまま総譜 PDF に (CI で実コンパイル検証済み)
