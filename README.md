...
**16 本の録音を実音源 (サンプラー) にして、《フーガの技法》の未完の四重フーガ Contrapunctus XIV (BWV 1080/19) の主題を 12 音階の全 12 調 × {正行, 反行} = 24 曲のフーガに展開し、ピアノロール付き MP4 に書き出す。** B-A-C-H が長 3 度ずつ 3 回で 12 音を敷き詰めること、ニ短調で **B♭ と C♯ が F を軸とする鏡像** であること、5 度圏が 12 調を一周することを「音階の秘密の原理」として作曲・反行・転調の規則にした。作曲エンジン・サンプラー・MIDI・動画生成は [`bach_fuga24/`](bach_fuga24/) を参照。
