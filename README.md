...
| **`bada_cinevim/`** | 🎞 **Bada CineVim** — 映画カウントダウンの VIM エディタ · どアップ フォーカス(ガウス減衰の被写界深度) · **イベント処理のフラッシュ効果**(全操作をイベントバスへ投入し、ディスパッチされた行が種別の色で発光 + 処理の光が行を走る + イベントバス HUD とスループット計) · アカデミー・リーダーの秒読み(毎秒 TICK / T-0 で全画面フラッシュ) · 本物のモーダル編集(モーション/オペレータ/レジスタ/`.` リピート/Ex) · 量子 Bada 実行系(状態ベクトル 14 qubit) · APK/EXE/AppImage 配布 | [→ 開く](bada_cinevim/) |
