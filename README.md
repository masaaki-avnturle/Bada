...
## 📖 ダウンロード — Bada 瞬読(速読スタジオ / 何でも取り込み + 多行ブロック速読 + 読み手の見え方 + 録画)

**今までの集大成 ── 提出してきた論文集と、画像処理トランスフォーマーの機知を一つにして、対象者の本を「文字の固まり」と「多行のブロック」で速読させ、その本文が読み手にどう見えているかを画像として作り、そのまま録画するアプリケーション。** 本は **PDF でも画像でも docx・odt・pptx・xlsx・epub・zip でも、tex・html・rtf・srt・csv・json でも、そのほか何の拡張子でも**取り込める ── 依存ゼロ・オフラインのまま受けるために、**deflate の展開も ZIP も PDF の読み出しも内蔵**してある(PDF は頁木の順に **ToUnicode CMap** で符号を文字へ戻し、1 バイト符号と 2 バイト符号の両方を解く。文字のない走査 PDF からは**埋め込み JPEG を頁の画像として**取り出す)。判定は拡張子より中身(魔法数)を優先し、どれにも当てはまらなくても文字として読めれば本文として受ける。 頁の写真やスキャンを取り込むと、**Swin 型の窓自己注意**(パッチ格子を W×W の窓に区切り、次の層で窓を半分ずらして境界を跨がせる)と**注意ロールアウト** Â⁽ˡ⁾ = 行正規化(½(A⁽ˡ⁾+I))、R = Â⁽ᴸ⁾⋯Â⁽¹⁾ が版面を解き、**文字の固まり → 行 → 多行ブロック → 読み順**を復元する。相対位置バイアス B(i,j) = −(α|Δ行| + β|Δ列|) は**等方ではなく**、横書きなら同じ行の隣を、縦書きなら同じ列の隣を遠くまで見に行く ── 文章は方向をもつ、という事実をそのまま注意の形にしてある。前処理は積分画像 + **Sauvola 適応二値化** T = μ(1 + k(σ/R − 1)) で本の写真の照明の傾きを吸収し、注意が張った辺は古典の **RLSA** の隣接と論理和を取って(注意だけに賭けず)二段組のノドを跨がない。書字方向は「**行間は字間より広い**」という組版の事実で判定し、縦書きなら読み順は右の列から左へ。**本アプリは文字認識を行わない** ── 速読に要るのは「どこからどこまでが一つの固まりか」であって、それが何という字かではないからである。復元した固まりは、対象者の「1 ブロックの固まり数 × 行数」で束ねられ(**多行ブロック** = 複数行を一度に見る面読みにも対応)、t(u) = max(t_min, 60000·chars/cpm_eff) の滞留で順に送られる。**そして画面に映すのは読み手の姿ではなく、「いま読み手には、この本文がどう見えているか」の像である。** 人が一度に受け取れるのは注視点のまわりだけで、離れるほど解像力は落ちる(皮質拡大則: ぼけ = max(0, (ecc − f₀)/E₂)、f₀ = 1 + 0.15·視野幅、E₂ = 0.8·視野幅)。その範囲は読み進む向きに**非対称**で(McConkie & Rayner の移動窓 ── 横書きなら右へ広く左へ狭く、縦書きなら下へ広く上へ狭い)、行をまたぐ向きには「取れる行数ぶんの平地」から先で一気に効く(ecc の行成分 = max(0, |行距離|/行送り − (0.35 + (取れる行数−1)·0.5)) × 9)。さらに**跳んでいる最中は見えていない**(サッカード抑制)。頁をぼけの段ごとに作り置きし、**等離心率の楕円**(この式の厳密な逆写像)のやわらかいマスクで鮮明な段ほど内側に重ねる。周辺は暗くなるのではなく**頁の平均色に溶ける** ── 字があることは分かるが字として見分けられない、という状態そのものである。だから、**1 行しか取れない読み手に 3 行のブロックを送っても「見えていない」ことが画像にそのまま出る**。カメラは**視線を取るためだけ**に使い(同じ窓自己注意エンジンで顔と目を掴み、停留・サッカード・逆行・瞬き・追従率を数え、読んだ字数÷停留回数で測れた視野幅を対象者へ返す)、**その像は画面にも録画にも端末の外にも出ない**。テキストを与えたときは 1 個ずつ流すのではなく**実際に頁へ組版してから**視野をかける ── 人が読んでいるのは頁だからである。**この画面はそのまま `captureStream()` + `MediaRecorder` で WebM 動画になる。**

### 👉 [**bada_sokudoku/index.html をダウンロード**](bada_sokudoku/index.html)
...
ビルドは [`sokudoku-app-build.yml`](.github/workflows/sokudoku-app-build.yml) が実行します(`sokudoku-v*` タグで Release へ添付 / `workflow_dispatch` で Actions アーティファクト)。解析の式・検査 263 項目・使い方は [`bada_sokudoku/`](bada_sokudoku/) を参照。
...
| **`bada_sokudoku/`** | 📖 **Bada 瞬読(速読スタジオ)** — **PDF / 画像 / docx / odt / pptx / xlsx / epub / zip / tex / html / rtf / srt / csv / json ほか何の拡張子でも取り込み**(deflate・ZIP・PDF の読み出しを内蔵。PDF は ToUnicode CMap で文字を復元し、走査頁は埋め込み JPEG を取り出す) · 対象者の本の頁を画像処理トランスフォーマー(Swin 型の窓自己注意 + ずらし窓 + 注意ロールアウト)で解析し、文字の固まり → 行 → **多行ブロック** → 読み順を復元(文字認識なし) · Sauvola 適応二値化 + RLSA 融合 · 縦書き/横書き自動判定と XY-cut · **画面に映すのは読み手の姿ではなく「その本文が読み手にどう見えているか」**(中心窩の解像力 + 読み進む向きに非対称な知覚スパン + 行方向の平地 + サッカード抑制 → 周辺は頁の平均色に溶ける) · カメラは視線を取るためだけに使用(停留/逆行/瞬き/追従率 → 対象者プロファイルへ。像は外に出ない) · 本文は頁に組版してから視野をかける · **canvas captureStream + MediaRecorder で「読み手に見えていたとおりの本文」を録画** · 論文 (.tex) を教材にする読み込み口 · APK/EXE/AppImage 配布 | [→ 開く](bada_sokudoku/) |
