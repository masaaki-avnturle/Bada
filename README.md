...
**量子プログラミング言語 Bada 用の、映画的な VIM エディタ。** **ポインタやカーソルが指した位置が、その場でどアップ**になります — **拡大鏡は要りません**。**右へ動かせば どアップ は右へ滑り、左へ戻せば左へ戻ってきます。** 1 行を語のまとまり(セグメント)に切り分け、**縦は行の距離・横は px の距離**で同じガウス式 `scale = 1 + (zpeak−1)·exp(−d²/2σ²)` を掛け合わせた二軸の魚眼です。距離は必ず**拡大していないときの位置**で測るので、拡大が自分の焦点を押しのけて振動することがありません(ポインタの位置は `displayToNatural()` で表示位置から自然位置へ逆に解きます)。拡大したぶんの余白は `offsets[i] = Σ_{j<i} size_j(k_j−1) + size_i(k_i−1)·anchor` で前後へ配り直され(**縦は中心基準 0.5・横は始点基準 0**)、**どれだけ拡大しても文字どうしが重なりません**。マージンを動かさないので毎フレームの再レイアウトも起きません。内側は行のままなので **VIM のモーションもオペレータもそのまま**効き、**日本語入力(IME)にも対応**しています。`:code` に切り替えると被写界深度のコードビューになります。操作はすべて**イベント**としてイベントバスへ流れ、**ディスパッチされた行がフラッシュ**します。
