...
## 📖 ダウンロード — Bada 瞬読(速読スタジオ / 画像処理トランスフォーマー + 読み手の捕捉 + 録画)

**今までの集大成 ── 提出してきた論文集と、画像処理トランスフォーマーの機知を一つにして、対象者の本を「文字の固まり」と「文章のブロック」で速読させ、その速読している映像をそのまま録画するアプリケーション。** 頁の写真やスキャンを取り込むと、**Swin 型の窓自己注意**(パッチ格子を W×W の窓に区切り、次の層で窓を半分ずらして境界を跨がせる)と**注意ロールアウト** Â⁽ˡ⁾ = 行正規化(½(A⁽ˡ⁾+I))、R = Â⁽ᴸ⁾⋯Â⁽¹⁾ が版面を解き、**文字の固まり → 行 → ブロック → 読み順**を復元する。相対位置バイアス B(i,j) = −(α|Δ行| + β|Δ列|) は**等方ではなく**、横書きなら同じ行の隣を、縦書きなら同じ列の隣を遠くまで見に行く ── 文章は方向をもつ、という事実をそのまま注意の形にしてある。前処理は積分画像 + **Sauvola 適応二値化** T = μ(1 + k(σ/R − 1)) で本の写真の照明の傾きを吸収し、注意が張った辺は古典の **RLSA** の隣接と論理和を取って(注意だけに賭けず)二段組のノドを跨がない。書字方向は「**行間は字間より広い**」という組版の事実で判定し、縦書きなら読み順は右の列から左へ。**本アプリは文字認識を行わない** ── 速読に要るのは「どこからどこまでが一つの固まりか」であって、それが何という字かではないからで、表示するのは**どこを見るか**だけである。復元したブロックは t(u) = max(t_min, 60000·chars/cpm_eff) の滞留で順に照らされ、視点は固まりから固まりへ渡る。**そして同じ窓自己注意エンジンを、こんどはカメラの向こうの「読んでいる対象者」そのものに向ける。** 肌色 (YCbCr の色差規則)・暗さ・勾配・前フレームとの差 (動き) の 10 次元パッチ特徴を取り、頁の「墨密度」の代わりに**肌色密度**を密度として同じ連結成分の抽出にかけると、いちばん大きな成分が**顔**になる。顔の上 18〜58% の帯の暗さの谷から**目**を取り、目の縦の広がりが**開き具合**(落ちた瞬間が瞬き)、視線指標 = clamp(1.6·瞳の顔の中での偏り + 0.8·頭の偏り) を読み進む向きに符号を合わせて (前面カメラは左右が逆なので反転) 追い、**停留・サッカード・逆行(読み進む向きと逆へ跳んだもの。縦書きなら上へ、あるいは右の列へ)・瞬き・追従率**を数え、読んだ字数 ÷ 停留回数で**その場で測れた視野幅**を対象者プロファイルへ書き戻す。**捉えた読み手の像は顔枠・目・視線の矢・注意の面を重ねたまま速読画面に合成されるので、`captureStream()` + `MediaRecorder` の録画には「本のどこが照らされていたか」と「そのとき読み手がどこを見ていたか」が同時に残る。**(学習済みの顔検出器は積んでいない。像も測定値も端末の外へ出ない。) 瞬読トレーナー (C) から**瞬読フラッシュ**と**語中シャッフル**を、`sokudoku/reports/` の提出論文 (.tex) から**論文そのものを教材にする読み込み口**を継承。依存ゼロ・単一 HTML・オフライン動作。
...
ビルドは [`sokudoku-app-build.yml`](.github/workflows/sokudoku-app-build.yml) が実行します(`sokudoku-v*` タグで Release へ添付 / `workflow_dispatch` で Actions アーティファクト)。解析の式・検査 144 項目・使い方は [`bada_sokudoku/`](bada_sokudoku/) を参照。
...
| **`bada_sokudoku/`** | 📖 **Bada 瞬読(速読スタジオ)** — 対象者の本の頁を画像処理トランスフォーマー(Swin 型の窓自己注意 + ずらし窓 + 注意ロールアウト)で解析し、文字の固まり → 行 → ブロック → 読み順を復元(文字認識なし) · Sauvola 適応二値化 + RLSA 融合 · 縦書き/横書き自動判定と XY-cut · 視野幅と目標速度から決まる滞留でブロックを順に照らすスポットライト速読 · **同じエンジンをカメラの向こうの読み手に向けて顔・目・視線を捉え画面に映す**(停留/サッカード/逆行/瞬き/追従率 → 対象者プロファイルへ) · **canvas captureStream + MediaRecorder で速読映像と読み手をまとめて録画** · 論文 (.tex) を教材にする読み込み口 · APK/EXE/AppImage 配布 | [→ 開く](bada_sokudoku/) |
