...
**量子プログラミング言語 Bada 用の、映画的な VIM エディタ。** **多段落の文章**を段落に分けて折り返して組み、**ポインタが拡大鏡の役割**をします。ポインタを動かすと**その下の文章だけがレンズの中でどアップ**になり、まわりの段落は静かに退きます。見かけの拡大ではなく、本体と同じ幅・同じ書体の複製を `loupeTransform(px,py,r,z) → {scale:z, tx:r/z-px, ty:r/z-py}` の写像で拡大する**本物の拡大鏡**で、どんな倍率でも指した点がレンズ中心に来ます。ホイールで 1.6×〜7×、指でなぞれば APK でも拡大鏡になります。ポイントされた**語**または**文**だけを切り出して明るくし(既定は文・行をまたいでも 1 文として扱う)、和文は詰めて欧文は空白で行を継ぎます。内側は行のままなので **VIM のモーションもオペレータもそのまま**効きます。`:code` に切り替えると、カーソル行をガウス核 `k = exp(-d²/2σ²)` で最大 2.6 倍に拡大する**被写界深度**のコードビューになります。操作はすべて**イベント**としてイベントバスへ流れ、**ディスパッチされた行がフラッシュ**します。1 キー入力 = 1 ディスパッチで、`key` → `motion` / `edit` / `ex` / `search` → `mode` の順にバスへ積まれ、処理された行が**種別の色で発光**し、**処理の光が行を左から右へ走り**、左端に**イベントマーカー**が立ちます。包絡は信号ランプと同じ `env = shape · (1-t) · intensity`(`t=0.12` でピーク、指数減衰)。画面左下の**イベントバス HUD** に直近のディスパッチが積まれ、右下に**スループット (ev/s)** と累計件数が出ます。`:countdown 5` で**アカデミー・リーダー**(同心円 + 掃引指針 + フィルムグレイン)が起動し、**毎秒 `TICK` イベント**、**T-0 で全画面フラッシュ**。編集は本物のモーダル VIM — `hjkl w b e f t % { }` のモーション、`d c y > <` のオペレータとカウント、名前付きレジスタ、`u` / `Ctrl-R` / `.` リピート、ビジュアル、`:%s/pat/rep/g` などの Ex コマンド。**量子 Bada ランタイム**(状態ベクトルシミュレータ / `qubit` `H` `X` `CNOT` `measure` / `<-` `-<` `>-` / β(p,q)・Γ(s))を `:run` で実行できます。依存ゼロ・単一 HTML・オフライン動作。
...
| **`bada_cinevim/`** | 🎞 **Bada CineVim** — 多段落の文章を読む拡大鏡エディタ · **拡大鏡ポインタ**(ポインタの下の文章がレンズの中でどアップ / 語・文の切り出し / ホイールで 1.6×〜7×) · どアップ フォーカス(ガウス減衰の被写界深度・コードビュー) · **イベント処理のフラッシュ効果**(全操作をイベントバスへ投入し、ディスパッチされた行が種別の色で発光 + 処理の光が行を走る + イベントバス HUD とスループット計) · アカデミー・リーダーの秒読み(毎秒 TICK / T-0 で全画面フラッシュ) · 本物のモーダル編集(モーション/オペレータ/レジスタ/`.` リピート/Ex) · 量子 Bada 実行系(状態ベクトル 14 qubit) · APK/EXE/AppImage 配布 | [→ 開く](bada_cinevim/) |
