...
**量子プログラミング言語 Bada 用の、映画的な VIM エディタ。** **多段落の文章**を文のブロックに分けて組み、**ポイントされたブロックがその場でどアップ**に開きます — **拡大鏡は要りません**。倍率は焦点からの距離 `d`(ブロック数)のガウス核 `scale = 1 + (bpeak−1)·exp(−d²/2·bspread²)` で決まり、`offsets[i] = Σ_{j<i} h_j(s_j−1) + h_i(s_i−1)/2` のずらし量で**拡大しても上下が重なりません**(マージンを動かさないので毎フレームの再レイアウトも起きません)。読み進めると、焦点が移った瞬間からそこを起点に**進行波**が伝わり、`x = amp·sin(k|d| − ω·t·2π)·exp(−|d|/λ)·(1−t)^1.6` で**文章が波打って、波の腹がフラッシュ**します。**ブロックは段落をまたいで一続き**なので、`}` `{` で**段落を移動しても**同じように波が広がります。行頭は動かず、段組の幅は倍率から逆算して組まれるので、いちばん大きく開いたブロックでも画面から出ません。内側は行のままなので **VIM のモーションもオペレータもそのまま**効き、**日本語入力(IME)にも対応**しています。`:code` に切り替えると被写界深度のコードビューになります。操作はすべて**イベント**としてイベントバスへ流れ、**ディスパッチされた行がフラッシュ**します。
