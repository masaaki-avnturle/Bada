...
## ⚛🔐 ダウンロード — Bada QCrypt(量子暗号ソフト / Bada 言語版)

**これまでの論文とアプリケーションで積み上げた量子暗号を、「暗号処理そのものを量子プログラミング言語 Bada で書く」形に作り換えたソフト。** 旧版が BB84 を確率則で模擬していたのに対し、本版は **BB84 量子鍵配送・ワンタイムパッド・検疫タグのすべてが Bada のプログラム**として同梱され、光子 1 個ずつが実際に状態ベクトル実行系のゲート列(X / H / Z / CNOT / 測定)を通ります。乱数すら `H q` → `measure q` の**量子乱数**。**🧪 量子検疫室**では大切なテキスト・ファイルを**量子封印**し、ウイルス感染を含む**あらゆる改変を 1 ビットから検出**(何バイト変わり最初の改変がどこかまで提示)して、**封印時の清浄な状態へバイト単位で復元**します。ウイルスの種類・手口を問わないのは、正体を調べるのではなく**「清浄だったときの姿」を基準に差分を見る**方式だからです(⚠ ただし万能なウイルス判定は決定不能 — F. Cohen, 1987。保証対象は**封印済み**のものです)。盗聴者 Eve を挿入すると複製不可能定理により QBER ≈ 25% が現れ、**QBER > 10% で検出して鍵を破棄**。「⚛ Bada プログラム室」では暗号処理の Bada ソースをその場で読み・編集・実行できます。依存ゼロ・単一 HTML・**完全オフライン**(通信機能を持ちません)。

### 👉 [**bada_qcrypt/index.html をダウンロード**](bada_qcrypt/index.html)
