...
## 🎼 ダウンロード — MusicTeX Studio(スペクトルコード制作 / texlive-music)

**画面構成は macOS デスクトップ専用アプリ風**: 信号機ボタン付きタイトルバー、トランスポート(▶/◼)+LCD のツールバー、左ライブラリ(楽器17種・テンプレート)、中央エディット領域、右インスペクタ(スペクトルコード縦フェーダー・五行コード線・⚛量子作曲)の3ペイン。

**🎻 オーケストラ多段譜**: 4パート(ヴァイオリン/フルート/トランペット/チェロ)の総譜表示(パートタブで各段を編集・段ごとに楽器切替)。譜面上部に**コード進行のバーコード音階譜**(構成音を12音階位置の波長色縦線で表示)、譜面クリックで**赤い縦の演奏ライン**が移動し、▶でそこから全パート同時演奏(ライン追従・自動スクロール)。`.tex` は `\instrumentnumber`+`&` の**多段オーケストラ MusixTeX** で書き出し。

**⚛ 量子作曲**: Bada VM Pro の**擬似量子コンピュータ・シミュレーター**(状態ベクトル・最大8量子ビット)と互換の**量子 Bada 言語**(qubit/H/X/Z/CNOT/measure/state/let/print)に音楽命令 `note`/`chord`/`rest` を追加。重ね合わせと量子もつれの測定結果が現在の調の音階・五行コード線に写像され、そのまま譜面化・スペクトル映像化・シンセ演奏されます。

**MusixTeX (texlive-music) の楽譜ソースを作成・ライブプレビュー・試聴し、`.tex` として書き出す楽譜制作アプリ。** **🎹 シンセサイザー(スペクトルコード)**: 倍音1〜8の振幅スペクトルをスライダーで設計する加算合成シンセ — **13楽器**(ピアノ/オルガン/ヴァイオリン/チェロ/フルート/トランペット/クラリネット/ギター/ハープ/オルゴール/ベル/マリンバ/ベース)+基本波形、各楽器は倍音+ADSR+ビブラートまで切替。**曲ファイルの取り込み**は「置き換え/重ねる」を選べ、外部の曲(mid/wav/mp3/ogg/m4a/flac/aac/opus/mp4等)を**作曲中の楽譜に被せたり**、曲のスペクトル分布を倍音コード化してシンセの音色に自動設定したり、小節ごとのコード進行を抽出して伴奏として重ねられます。**五行コード線** = M/m/7/M7/sus4 の5行×12音階のコード格子をクリックすると、シンセで鳴りながら調号を考慮した和音が楽譜に書き込まれ、出力はライブスペクトル(12音階の色のFFT)として映像化されます。**🌈 モーツァルト・ビジョン**: モーツァルトが音を色として視て作曲した共感覚を物理対応で具現化 — 音の周波数を約40オクターブ上げると可視光の周波数帯に一致する対応(A4 440Hz → 484THz ≒ 620nm 赤)で、楽譜を**色の波長帯の映像**として表示(試聴と同期して発光)し、**波長で着色された色鍵盤**をクリックして色から曲を制作できます。**曲ファイルを入れると楽譜を自動で記述(自動採譜)**: MIDI (.mid) は SMF 解析(和音対応)、音声 (wav/mp3/ogg/m4a) は自己相関ピッチ検出(単旋律)で MusixTeX を生成し、調号・音価・休符・小節割りまで自動で付けます。レポート **「Δと」(octave74)** の音階理論(B♭・E♭ 長音階のとりうる音、2音階=2オクターブ、G と A の和音、HANON)をテンプレートとして同梱。依存ゼロ・単一 HTML・オフライン動作。印刷用 PDF は texlive-music の `musixtex -p score.tex` で組版します。
