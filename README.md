...
## 🪙 ダウンロード — 小槌の家計簿(レシート撮影で自動記帳・複式簿記)

**レシートをスマホ・タブレットのカメラで撮影するだけで、自動でデータ登録される簿記機能付き家計簿。** OCR(Tesseract.js・ブラウザ内実行で画像は端末外へ送信しない)がレシートの**日付・店名・品目・金額**を読み取り、品目名と店名から**勘定科目を自動判定**(食費/外食費/日用品費/医療費 など)して複式簿記の仕訳を自動作成。**仕訳帳・総勘定元帳・合計残高試算表・損益計算書(P/L)・貸借対照表(B/S)**・費目別グラフを備え、現金/クレジット/電子マネー/預金の支払方法に対応。手入力・JSON バックアップ・仕訳帳 CSV 書き出しも可能。データは端末内(localStorage)にのみ保存。単一 HTML。

### 👉 [**kozuchi_kakeibo/index.html をダウンロード**](kozuchi_kakeibo/index.html)
