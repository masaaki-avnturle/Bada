...
## 🎹 ダウンロード — Bada Music Studio(音楽制作ソフト / DAW)

**市販の DAW と同じ作り方で曲が作れる、依存ゼロの音楽制作ソフト。** 打ち込み・音づくり・ミックス・書き出しまでを 1 本の HTML で完結します。**トラック × 小節のアレンジ**(クリップの配置・反復・移動)、**ピアノロール**(描画 / 長さ変更 / ベロシティレーン / クオンタイズ / 9 種のスケール表示 / コードモード)、**ステップシーケンサ**(キック・スネア・クラップ・リム・HH・タム・ライド・クラッシュの **10 音源をすべてその場で合成** — サンプル不使用、1/8〜1/32 と 3 連、ステップ別ベロシティ、ビート自動生成)、**減算合成シンセ**(2 オシレータ + サブ + ノイズ、最大 5 声ユニゾン、4 モードフィルタ + エンベロープ + キートラック、ADSR ×2、LFO、**13 プリセット**)、**ミキサー**(フェーダー / 等パワーパン / ミュート / ソロ / レベルメーター)、**エフェクト**(ドライブ・3 バンド EQ・コーラス + リバーブとテンポ同期ピンポンディレイのセンド + マスターリミッタ)。

書き出しは **WAV**(`OfflineAudioContext` による**オフラインレンダリング** — 16/24 bit PCM・32 bit float、44.1/48/96 kHz、ノーマライズ)、**ステム**(トラック別 WAV)、**MIDI**(SMF format 1 の書き出し / 読み込み)、**プロジェクト**(`.badaproj` JSON + ブラウザ自動保存)。オーディオファイルの読み込みとマイク録音にも対応。**再生と書き出しはまったく同じ音声グラフを共有する**ので、聴こえたとおりの音が書き出されます。起動するとデモ曲が入っているので ▶ を押せばすぐ鳴ります。依存ゼロ・単一 HTML・完全オフライン(曲が端末の外に出ることはありません)。

### 👉 [**music_studio/index.html をダウンロード**](music_studio/index.html)
...
ビルドは [`musicstudio-app-build.yml`](.github/workflows/musicstudio-app-build.yml) が実行します(`musicstudio-v*` タグで Release へ添付 / `workflow_dispatch` で Actions アーティファクト)。使い方・音源の仕組み・ショートカットは [`music_studio/`](music_studio/) を参照。
