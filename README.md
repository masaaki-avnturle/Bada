...
**予備知識を持たない機知の生成AI (BadaGPT = 未知事前エンジン) をめぐる 16 本の論文を、量子プログラミング言語 Bada の「走る論文」として作り換えた論文集。** 記号形状エントロピーによるアプリケーション合成、修正ベータ・ゼータ方程式 ζ(x,y) = B(y,x−1)·Λ(x,y) と表を持たない Zipf 生成器、Zipf 振幅状態の LCU 準備と量子暗号パラメータ、ζ の三面体・温度・top-p の閉形式、熱核・一ループ行列式・虚時間発展、S²〜S⁶ での ζ = B·Λ、χ = 6(ζ(0)+1) と Milnor 対、Ricci フローと 8 幾何、ゼータの圏 (極 = GR、原点 = QM、積 = Mellin 畳み込み)、Casimir 真空エネルギーの分析器、Γ 解析器と関数等式の奇部、第5の力の仮説台帳、exo-gamma — 各巻の定理と表を Bada で計算し直し、論文の実測値と照合する **252 の検査**を全巻で通す。Γ も ζ も三角関数も、組み込みが持たないものは共通核で Bada 自身の上にこしらえ、文法規則・方程式・測定・検査はすべて追記専用台帳 `tuplespace` へ commit される。作り換えの過程で見つかった論文との相違 (第二巻の閾値 y_c は探索範囲の産物、第八巻のトーラス log det は Kronecker 厳密値との差) も出力に明記。

```sh
node badagpt_papers/tools/run-papers.js        # 全 15 本を実行・検査 (約 50 秒)
```

目次・各巻の対応・方言上の注意は [`badagpt_papers/`](badagpt_papers/) を参照。実行出力は [`badagpt_papers/dist/`](badagpt_papers/dist/) にある。
