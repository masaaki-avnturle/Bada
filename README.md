...
| **`bada_sokudoku/`** | 📖 **Bada 瞬読(速読スタジオ)** — 対象者の本の頁を画像処理トランスフォーマー(Swin 型の窓自己注意 + ずらし窓 + 注意ロールアウト)で解析し、文字の固まり → 行 → **多行ブロック** → 読み順を復元(文字認識なし) · Sauvola 適応二値化 + RLSA 融合 · 縦書き/横書き自動判定と XY-cut · **画面に映すのは読み手の姿ではなく「その本文が読み手にどう見えているか」**(中心窩の解像力 + 読み進む向きに非対称な知覚スパン + 行方向の平地 + サッカード抑制 → 周辺は頁の平均色に溶ける) · カメラは視線を取るためだけに使用(停留/逆行/瞬き/追従率 → 対象者プロファイルへ。像は外に出ない) · 本文は頁に組版してから視野をかける · **canvas captureStream + MediaRecorder で「読み手に見えていたとおりの本文」を録画** · 論文 (.tex) を教材にする読み込み口 · APK/EXE/AppImage 配布 | [→ 開く](bada_sokudoku/) |
