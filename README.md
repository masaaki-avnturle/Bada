...
## 🧊 ダウンロード — こんにゃく照合器(方程式の形 × 数値のエントロピー照合)

**手帳の見開き — 左ページの方程式と絵、右ページの箱・段落・三本線 ≡ — を、そのままダイアログボックス操作のアプリに作り換えたもの。** 4 つの箱(段落)をひらいて任意の数値を入れると、後ろ側で **絵の形のエントロピー**(鞍 / 軸 / 葉 / 閉 の重み)と **入力数値のエントロピー**(4 ビン線形補間)を照合し、照合率・**こんにゃく弾力性**(つまんだときの戻り)・**優しさ**(数列の跳ね)を測って、残差を τ=0.25 bit で丸めた **同値類 ≡** に箱を束ねます。三条件の門(**優しさのカイン**)を欠けると「カインの刻印」が理由つきで残り、通っても通らなくても SHA-256 ハッシュ連鎖の**アカシックレコード**(追記専用台帳・検証/JSONL 書き出し)に綴じられます。依存ゼロ・単一 HTML・オフライン動作。

### 👉 [**omega_konnyaku_dialog/index.html をダウンロード**](omega_konnyaku_dialog/index.html)
