...
「**既存のパッケージと競合しているためインストールできません**」と出る場合、原因は
**署名の不一致**です。以前のビルドは CI が毎回別のデバッグ鍵で署名していたため、上書き
インストールが競合していました。

**恒久対策（済）**：固定のデバッグ鍵（`app/debug.keystore`）を導入したので、**今後の全ビルドは
同一署名**になり競合しません。

**いま競合している端末での対処（一度だけ）**：
1. 端末から**古い版をアンインストール**（設定→アプリ→ NoemaKey / 未知事前予知エンジン を削除）。
   ※ 旧 `com.yamaguchi.bada` 版が残っていれば、それも削除。
2. 最新の `noema-precog.apk` を入れ直す。以降のバージョンは競合せず上書き更新できます。
