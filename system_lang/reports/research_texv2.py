こちらは、指定した CVS ルート配下の `.tex,v` ファイルを再帰走査して、「語句（固定文字列 or 正規表現）」および「数式（LaTeX 形式の簡易検出）」を検索するコマンドライン Python スクリプトです。オプションで出力を CSV に保存したり、スニペット付きで結果を標準出力できます。
...
注意事項
- `.tex,v` は RCS のリビジョン保存フォーマットです。直接中身を検索するのは可能ですが、RCS の構造（ヘッダ／デルタ）により本文が埋もれていることがあります。より正確にはワーキングコピーの `.tex` ファイルを検索するか、RCS の内容を解凍して本文を抽出してから検索する方法を検討してください。
- 文字コード問題がある場合は `--encoding` を変更してください（例: `shift_jis`）。
- 数式検出は簡易判定です。必要なら正規表現を強化してください（例: 複雑な環境や MathML を含める等）。

必要なら、
- RCS の本文抽出（`,v` から最新のテキストだけを取り出す）を行うバージョンを作りますか？その場合サンプル `.tex,v` を一つ提供してください。
