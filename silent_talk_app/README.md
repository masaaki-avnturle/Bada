...
## 🎙️ サイレント・リスニング（聞き取り開始/停止 + 合言葉コマンド）

「聞き取り開始」で Silent Talk が起動し、マイクから**小声/ささやき**を
連続音声認識します。「聞き取り停止」で停止します。認識フレーズが**合言葉**に
一致するとコマンドを実行し、一致しなければ入力欄へ追記して無音信号化します。

- **起動の合言葉**: 聞き取り開始 / サイレント開始 / start listening
- **停止の合言葉**: 聞き取り停止 / サイレント停止 / stop listening
- **コマンド例**: 符号化 / 復号 / クリア / キーボード / センサー / ヘルプ

> ⚠️ **正直な技術的制約**: 通常のスマホ/タブレットのハードウェアでは、
> 頭の中で黙読した言葉や思い浮かべたイメージを**直接読み取ることはできません**
> （それには筋電(EMG)や脳波(EEG)などの専用機器が必要です）。本アプリは
> 実機で動作する最も近い形として、**マイク経由の小声(subvocalization 近似)**を
> 音声認識します。音声はネットワークへ送らず端末内で処理されます（権限はマイクのみ）。
