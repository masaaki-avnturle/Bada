...
static void token_set_str(Token *t, TokType ty, const char *s){
  ...
  static void lex_next(Lexer *lx){
    if(!lx) return;
    const char *s = lx->src ? lx->src : "";
    int i = lx->pos;
    if(lx->cur.s){ free(lx->cur.s); lx->cur.s = NULL; }
    ...
    default:
      log_error("lex_next: unexpected character 0x%02x at pos %d; skipping", (unsigned char)c, i);
      lx->cur.type = TK_EOF; return;
    }
  }
  ...
  static Ex *parse_primary(Parser *p){
    Lexer *lx = p->lx;
    Token cur = lx->cur;
    if(cur.type == TK_NUM){ Ex *e = ex_new_num(cur.num); lex_next(lx); return e; }
    if(cur.type == TK_STR){ Ex *e = ex_new_str(cur.s ? cur.s : ""); lex_next(lx); return e; }
    if(cur.type == TK_IDENT){
      char *name = xstrdup_safe(cur.s ? cur.s : "");
      lex_next(lx);
	...
	if(lx->cur.type != TK_RP){
	  while(1){
	    Ex *arg = parse_expr(p);
	    args = xrealloc(args, sizeof(Ex*)*(nargs+1)); args[nargs++] = arg;
	...
	while(lx->cur.type == TK_STR || lx->cur.type == TK_NUM || lx->cur.type == TK_IDENT || lx->cur.type == TK_LP){
	  Ex *arg = NULL;
	  if(lx->cur.type == TK_LP){
	    lex_next(lx); arg = parse_expr(p); if(lx->cur.type == TK_RP) lex_next(lx);
	  } else if(lx->cur.type == TK_STR){ arg = ex_new_str(lx->cur.s); lex_next(lx); }
	  else if(lx->cur.type == TK_NUM){ arg = ex_new_num(lx->cur.num); lex_next(lx); }
	  else { arg = ex_new_ident(lx->cur.s); lex_next(lx); }
	  args = xrealloc(args, sizeof(Ex*)*(nargs+1)); args[nargs++] = arg;
	}
	Ex *fn = ex_new_ident(name);
	Ex *call = ex_new_call(fn, args, nargs);
