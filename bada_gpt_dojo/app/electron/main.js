/*
 * main.js — BadaGPT道場 の Electron ラッパー
 *          (Windows 10/11 EXE / Ubuntu AppImage・deb)
 *
 * アプリ本体は www/index.html に完全自己完結しています
 * (道場カリキュラム + BadaGPT パイプライン開示 + アプリ錬成 +
 *  量子 Bada インタープリタ + self-attention トランスフォーマー)。
 * ここではデスクトップウィンドウとして読み込むだけで、
 * 外部リンクは OS の既定ブラウザへ渡します。
 * 進捗 (印可・段位) は localStorage (Electron の userData 配下) に
 * 永続化されます。
 */
