...
**多段落の文章**を文のブロックに分けて組み、**ポイントされたブロックがその場でどアップ**に
開きます。**拡大鏡は要りません。** 読み進めると、焦点が移った瞬間にそこから外へ波が伝わり、
**文章が波打って、波の腹がフラッシュ**します。波は**段落をまたいでも**同じように広がります。
...
文書は**空行で段落**に、段落は**句点で文のブロック**に分かれます。ブロックは段落を
またいで一続きに並び、**ポイントされたブロックがその場で拡大**されます。
レンズも複製もありません。

### その場でどアップ

焦点からの距離 `d`(ブロック数)でガウス核が決まり、その場の倍率になります:

```
scale = 1 + (bpeak − 1) · exp(−d² / 2·bspread²)
```

拡大しても**上下が重ならない**ように、各ブロックのずらし量を求めます:

```
offsets[i] = Σ_{j<i} h_j(s_j − 1) + h_i(s_i − 1)/2
```

これはマージンを動かさず `transform` だけで済むので、**毎フレームのレイアウト
再計算が起きません**。「ブロック i の下端 = ブロック i+1 の上端」がちょうど一致
することはテストで検証しています。

**行頭は動きません**(`transform-origin: 0 50%`)。だからどれだけ拡大しても、
読み始めの位置はいつも同じところにあります。段組の幅は**倍率から逆算**して
組まれるので、いちばん大きく開いたブロックでも画面から出ません。

### 波打ち

焦点が別のブロックへ移った瞬間から、そこを起点に**進行波**が伝わります:

```
phase = k·|d| − ω·t·2π
x     = amp · sin(phase) · exp(−|d|/λ) · (1−t)^1.6
y     = x · 0.15                     ← 縦ゆれは行間に収まる範囲だけ
glow  = |sin(phase)| · exp(−|d|/λ) · (1−t)^1.6
```

距離とともに弱まり(`λ`)、時間とともに凪ぎます(`t = 1` で完全に停止)。
波の腹では `glow` が上がって**フラッシュ**します。

縦ゆれを横ゆれの 0.15 倍に抑えているのは意図的です。ブロックは上下に隙間なく
積まれているので、縦に大きく振ると文字どうしが重なります。**波の主役は横ゆれ**に
任せ、縦は行間に収まる範囲だけ動かしています。

### 段落を移動しても同じ

ブロックは段落をまたいで**一続きの列**なので、距離 `d` は段落の境目を意識しません。
`}` や `{` で段落を跳んでも、`j` `k` で一文ずつ進んでも、**同じように波が広がります**。

| 操作 | 効果 |
|:---|:---|
| **ポインタを動かす** | 触れたブロックがその場でどアップに |
| **`j` `k`** | 次のブロックへ焦点が移り、そこから波が伝わる |
| **`}` `{`** | 段落を移動。波は段落をまたいで広がる |
...
内側の構造は**行のまま**です。1 つの文が複数行にまたがっても、1 行に複数の文が
あっても、行の断片(`.src[data-l]`, `data-c0`)として保持されるので、
**VIM のモーションもオペレータもそのまま効きます**。
...
`:code` のコードビューで効く、行単位のどアップです(文章ビューでは上のブロックのどアップが担当します)。
...
### Ex コマンド
`:prose` `:code` `:wave` ・ `:set bpeak=2.4` `:set amp=30` `:set lambda=4` ・
...
**文章ビューでは触れた文章のブロックがその場でどアップ**になり、指を動かすと波が伝わります。
...
`index.html` のインライン `<script>` を Node の `vm` で読み込み、**502 件**を検証します —
...
カウントダウンの単調減少)、そして**多段落と文のブロック**(全行が段落か隙間に必ず
含まれること、和文は詰めて欧文は空白で継ぐこと、行境界のオフセットが次の行の先頭に
属すること、送り仮名を含む語の切り出し、小数点で文が切れないこと、**ブロックを連結
すると元の文章に戻ること**(取りこぼしなし)、行をまたぐ文の断片が正しい行と桁を
指すこと、**その場で拡大してもブロックが重ならないこと**、波が距離と時間で減衰し
`t=1` で完全に凪ぐこと、縦ゆれが行間に収まること、**波面が時間とともに外へ進むこと**)。
