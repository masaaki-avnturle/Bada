...
操作はすべて**イベント**としてイベントバスへ流れ、**ディスパッチされた行がフラッシュする**
— 処理されている場所が光で分かるエディタです。
...
**⌨** ボタンでソフトキーボードを呼び出せます。行をタップするとそこへ焦点が移って
`MOTION` イベント、同じ行をもう一度タップすると `SIGNAL` イベントが飛びます。
...
`index.html` のインライン `<script>` を Node の `vm` で読み込み、**317 件**を検証します —
