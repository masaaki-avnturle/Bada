...
**多段落の文章**を組んで表示し、**ポインタが拡大鏡の役割**をします。
ポインタを動かすと、**その下の文章だけがレンズの中でどアップ**になり、
まわりの段落は静かに退きます。操作はすべて**イベント**としてイベントバスへ流れ、
**ディスパッチされた行がフラッシュ**します。
...
## 🔍 拡大鏡ポインタ — 多段落の文章をどアップで読む

文書は**空行で段落に分かれ**、読みやすい幅(最大 74 文字)に折り返して組まれます。
その上を**ポインタが拡大鏡として動き**、レンズの下にある文章が拡大されます。

### 本物の拡大鏡

見かけの拡大ではありません。`#page` と**まったく同じ幅・同じ書体・同じ余白**の複製
`#pageMag` をレンズの中に置き、写像で拡大しています:

```
loupeTransform(px, py, r, z)  ->  { scale: z, tx: r/z - px, ty: r/z - py }
CSS: transform: scale(z) translate(tx px, ty px)
```

ページ座標 `(px, py)` にある点が、ちょうどレンズの中心 `(r, r)` に来ます。
折り返しが本体と一致しているので、**レンズの中の文章は本体と同じ行・同じ改行**で並びます。

| 操作 | 効果 |
|:---|:---|
| **ポインタを動かす** | レンズが追い、その下の文章がどアップになる |
| **ホイール** | 倍率を 1.6× → 2.2 → 3 → 4 → 5.5 → 7× で切り替え |
| **クリック / タップ** | そこへカーソルを移す(`MOTION` イベント) |
| **`j` `k` で移動** | レンズがカーソルを追いかける — キーボードだけでも読める |
| **指でなぞる** | APK / タブレットでは、指がそのまま拡大鏡になる |

### ポイントされた部分の切り出し

レンズの中心にある**語**または**文**だけを切り出して明るくします(既定は文)。

- **文** — 句点 `。`、`！`、`？`、欧文の `.` `!` `?` を手がかりに、いま読んでいる一文の
  始まりと終わりを探します。`3.14` の小数点は文末と見なさず、`」` `）` などの閉じ括弧は
  文末に含めます。**行をまたいでも段落内なら 1 文**として扱われます
- **語** — ひらがな / カタカナ / 漢字 / ラテン・数字 / 約物 をそれぞれ 1 類とし、
  同じ類の連なりを 1 語とします。漢字の直後のひらがなは**送り仮名**として同じ語に
  含めます(「動く」「文章は」)。辞書は使わないので、カタカナと漢字の境目では語が分かれます

和文の行は**詰めて継ぎ**、欧文が絡むところにだけ空白を入れます(日本語に余計な空白が
入りません)。

| コマンド | 効果 |
|:---|:---|
| `:loupe` | 拡大鏡の ON / OFF |
| `:prose` / `:code` | 文章ビュー(多段落)/ コードビュー(等幅) |
| `:set zoom=4` | 倍率 |
| `:set lensr=200` | レンズの大きさ(半径 px) |
| `:set lens=circle` | レンズの形(`circle` / `band`) |
| `:set snap=sentence` | 強調の単位(`sentence` / `word` / `off`) |
| `:set nodim` | まわりの段落を暗くしない |

内側の構造は**行のまま**なので、折り返して表示していても **VIM のモーションも
オペレータもそのまま効きます**。各行は `.src[data-l]` として段落の中に残っています。
...
`:code` のコードビューで効く、行単位のどアップです(文章ビューでは拡大鏡が担当します)。
...
### Ex コマンド
`:prose` `:code` `:loupe` ・ `:set zoom=4` `:set snap=word` `:set lens=band` ・
...
**⌨** ボタンでソフトキーボードを呼び出せます。**文章ビューでは指がそのまま拡大鏡**に
なり、なぞった場所がどアップになります。タップするとそこへカーソルが移って `MOTION`
イベントが飛びます。コードビューでは、同じ行をもう一度タップすると `SIGNAL` イベントです。
...
`index.html` のインライン `<script>` を Node の `vm` で読み込み、**402 件**を検証します —
...
カウントダウンの単調減少)、そして**多段落と拡大鏡**(全行が段落か隙間に必ず含まれること、
和文は詰めて欧文は空白で継ぐこと、行境界のオフセットが次の行の先頭に属すること、
送り仮名を含む語の切り出し、小数点で文が切れないこと、**どんな倍率でも指した点が
レンズ中心に来ること**、レンズのはみ出し防止、行をまたぐ文を行ごとに包んでも
文字が失われないこと)。
